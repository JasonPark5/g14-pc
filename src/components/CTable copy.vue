<template>
  <div>
    ...
    <q-table
      ...
      :separator="separator"
      :rows="props.rowKey ? reload.dataRows : props.data!"
    ...
    >
          ...
          <template v-for="col in viewHeader">
            ...
            >
              ...
              <span v-if="col.type === 'attach' && data && data.length > 0">
                <q-btn-group outline>
                  <q-icon name="help" color="primary" class="custom-btn" style="font-size: 1.4em">
              ...
              >
                <q-btn-group outline>
                  <q-icon
                    ...
                    class="inner-help-btn text-primary"
                  >
                    <q-tooltip anchor="bottom left" self="top left">
                      <div class="tooltipCustomTop">
                        {{ $language('설명') }}
      ...
      <template v-slot:body="bodyProps">
        ...
        <template v-if="!customDataTr">
          <!-- :props="{ ...props, key: `${props.key}_${uid()}` }" -->
          <q-tr
            :props="bodyProps"
            :class="[
              {
                'bg-light-blue-1': selectedRow(bodyProps),
                'table-tr-highLight': setHighLight(bodyProps),
                'no-hover-row': noHoverRow
              },
              customTrClass(bodyProps)
            ]"
            v-on:click="(e: MouseEvent) => rowClick(e, bodyProps.row, bodyProps.rowIndex)"
          >
            <!--
                앞에 배치됨
                ...
                <template v-if="contentsField(bodyProps, col)">
                  ...
                  <template v-if="editTdCheck(col.type)">
                    ...
                    <c-multi-select
                      ...
                      class="no-label-control"
                      :editable="editable"
                      :disabled="checkEnable(col, bodyProps.row, bodyProps)"
...
const reload = ref({
  dataRows: [] as Array<any>
})
const proxyColumns = ref<any>(null)
...
watch(
  ...
  () => {
    ...
    if (
      // props.data != reload.value.dataRows ||
      !isSameLength(props.data!, reload.value.dataRows) ||
      (Array.isArray(props.data) &&
        _.cloneDeep(props.data).length !== _.cloneDeep(reload.value.dataRows).length)
    ) {
      setGridData(reload.value, props.data!, 'dataRows')
    }
  },
  { deep: true }
)
...
function init() {
  ...
  reload.value.dataRows = props.data ? _.cloneDeep(props.data!) : []
}
/******************************
...
function closePopup(data: Array<any>) {
  ...
  if (data && data.length > 0) {
    if (setUserInfo.value.gubun === 'td') {
      ...
      acceptData(setUserInfo.value.row, setUserInfo.value.col.name, data[0].userName, [
        { name: setUserInfo.value.col.userId, value: data[0].userId },
        { name: deptCdText, value: data[0].deptCd },
        { name: deptNameText, value: data[0].deptName },
        { name: jobNameText, value: data[0].jobName },
        { name: spotNameText, value: data[0].spotName }
      ])
...
function deleteUserPop(row: any, col: any, gubun: string) {
  ...
  if (setUserInfo.value.gubun === 'td') {
    ...
    acceptData(setUserInfo.value.row, setUserInfo.value.col.name, '', [
      { name: setUserInfo.value.col.userId, value: '' },
      { name: deptCdText, value: '' },
      { name: deptNameText, value: '' },
      { name: jobNameText, value: '' },
      { name: spotNameText, value: '' }
    ])
    emits('table-data-change', { row: setUserInfo.value.row }, setUserInfo.value.col)
  } else {
...
/******************************
 * TODO (목적): 하이라이트 옵션을 넣은 경우 행 클릭에 따른 true/false 반환
 * @param (1): 행 정보
 * @return (반환): 행 클릭 여부
 *******************************/
function selectedRow(bodyProps: any) {
  if (!props.noHighLight) {
    return bodyProps.rowIndex === selectedRowIndex.value
  } else {
    return false
  }
}
...
/******************************
 * TODO (목적): [rowKey 사용 시] data 정보를 reload 중인 데이터와 치환
 * @param (1): reload 중인 데이터 (일반적으로 props.row가 해당이 됨)
 * @param (2): item의 key에 해당하는 정보
 * @param (3): item[key]에 할당할 값
 * @param (4): 그 외에 넣을 값으로 여러 개를 동시에 넣을 수 있음
 *******************************/
function acceptData(
  item: any,
  valueCol: string,
  value: string,
  others?: Array<{ name: string; value: any }>
) {
  if (props.rowKey && props.data && props.data.length > 0) {
    const dataIndex = _.findIndex(props.data, (dataItem: any) => {
      return dataItem[props.rowKey] === item[props.rowKey]
    })
    if (dataIndex > -1) {
      props.data[dataIndex][valueCol] = value
      if (others && others.length > 0) {
        _.forEach(others, (other) => {
          props.data![dataIndex][other.name] = other.value
        })
      }

      if (props.data[dataIndex]['editFlag'] !== 'C') {
        props.data[dataIndex]['editFlag'] = 'U'
        props.data[dataIndex]['chgUserId'] = user.value.userId
      }
    }
  }
}
 ...
 * @param (2): 컬럼 이름
 *******************************/
function headerDataChange(val: string, bodyProps: any, col: any, name?: string) {
  ...
  if (filterRows && filterRows.length > 0) {
    ...
    forEach(filterRows, (item) => {
      if (
        ...
        /** props.data 적용 */
        acceptData(item, valueCol, val, [{ name: nameCol, value: name }])
      }
    })
...
function datachange(bodyProps: any, col: any, type?: string) {
  ...
  acceptData(bodyProps.row, col.name, bodyProps.row[col.name], others)
...
function changeSelect(val: any, bodyProps: any, col: any) {
  ...
  if (userTypeCol && userTypeCol.isInout && val.value === userTypeCol.out) {
    ...
    acceptData(
      ...
      _.concat(
        [
          { name: userTypeCol.userId, value: null },
          { name: userTypeCol.field, value: null }
        ],
        _.map(userTypeCol.relationCols, (relationCol) => {
          return {
            name: relationCol,
            value: null
          }
    ...
    acceptData(bodyProps.row, col.name, val.value)
  }
  if (bodyProps.row['editFlag'] !== 'C') {
    bodyProps.row['editFlag'] = 'U'
    bodyProps.row['chgUserId'] = user.value.userId
...
function callbackProxy(data: any, bodyProps: any, col: any) {
  ...
  if (proxyColumns.value && proxyColumns.value[bodyProps.rowIndex]) {
...
function setPreview(file: any) {
  ...
  if (file.sysAttachFileId) {
    $http({
      ...
      preview.value.src = 'data:' + file.contentType + ';base64,' + _result.data
...
function taskChange(bodyProps: any, col: any, task: any) {
  if (col.type === 'vendor') {
    ...
    bodyProps.row[bizNo] = task.bizNo
    bodyProps.row[chargeName] = task.chargeName
    bodyProps.row[phoneNo] = task.phoneNo

    acceptData(bodyProps.row, col.name, bodyProps.row[col.name], [
      { name: vendorName, value: task.vendorName },
      { name: address, value: task.address },
      { name: bizNo, value: task.bizNo },
      { name: chargeName, value: task.chargeName },
      { name: phoneNo, value: task.phoneNo }
    ])
    ...
    acceptData(bodyProps.row, col.name, bodyProps.row[col.name], [
      { name: facilityName, value: task.facilityName }
    ])
    ...
    acceptData(bodyProps.row, col.name, bodyProps.row[col.name], [
      { name: equipName, value: task.equipName },
      { name: equipNo, value: task.equipmentNo }
    ])
  }

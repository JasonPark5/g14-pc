    ...
    >
      <q-card class="preview-image-card">
        ...
        <q-img
          ...
          :img-style="{ maxHeight: '80vh', width: 'auto', margin: '0 auto', display: 'block' }"
        />
        <q-card-section>
          <div class="row no-wrap items-center">
            <div class="col text-grey-9" style="font-size: 0.9em; font-weight: 500">
              {{ preview.oriFileNm }}
            </div>
        ...
        <q-card-section v-if="preview.description" class="q-pt-none">
          <span class="text-subtitle1" style="font-size: 0.9em">
            <q-icon name="info" class="q-mx-xs" style="position: relative; top: -2px" />
            {{ $language('파일 설명') }}
          </span>
          <span class="text-caption text-primary q-ml-sm">
            {{ preview.description }}
          </span>
        </q-card-section>
      </q-card>
...
import { extend, find, findIndex, forEach } from 'lodash-es'
import { storeToRefs } from 'pinia'
import { uid, useQuasar } from 'quasar'
import $format from 'string-format'
import { computed, inject, onBeforeMount, ref, watch } from 'vue'

import { decrypt, encrypt, fileDownDecrypt } from '@utils/aes256'
import { base64ToBlob, bytesToSize, getAccept } from '@utils/common'
...
function init() {
  ...
  largefileDownUrl.value = selectConfig.com.upload.largefileDown.url
  ...
  if (
    ...
    findIndex(formFields.value, { name: 'taskKey' }) > -1
  ) {
    const taskKeyData: any = find(formFields.value, { name: 'taskKey' })
 ...
 * @param (1): tempKey
 *******************************/
function setTaskKey(val: stringNull) {
  props.attachInfo.taskKey = val
}
/******************************
...
function setUploderSetting() {
  ...
  $http({
    ...
    if (_result.data) {
      ...
      extend(uploaderSetting.value, _result.data)
    }
  })
}
/******************************
    ...
    $http({
      ...
      if (_result.data) {
        ...
        if (beforeFileList.value && beforeFileList.value.length > 0) {
          forEach(beforeFileList.value, (item: any) => {
            const encstr = item.previewImage
            const url = window.URL || window.webkitURL
            const blob = base64ToBlob(encstr, 'image/jpeg')
            const imgUrl = url.createObjectURL(blob)
...
function fileDown(file: any) {
  ...
  if (file.fileSize <= 0) {
    $http({
      ...
      timeout: 0,
      maxContentLength: Infinity, // 응답 크기 제한 해제
      maxBodyLength: Infinity
    }).then((_result: any) => {
      const fileInfo = fileDownDecrypt(_result.data)
      // console.log('# fileInfo: ', fileInfo.id)
      // console.log('# file: ', file.sysAttachFileId)
    ...
    $http({
      ...
      //const fileId = decrypt(_result.headers['x-encrypted-file-id'])
      const fileId = decrypt(_result.config.params.sysAttachFileId)
      if (fileId === file.sysAttachFileId) {
        const url = window.URL.createObjectURL(new Blob([_result.data]))
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', file.oriFileNm)
...
function setPreviewFix(file: any, type: 'single' | 'multi') {
  ...
  if (file.sysAttachFileId) {
    ...
    $http({
      ...
      if (type === 'single') {
        previewfix.value.src = 'data:' + file.contentType + ';base64,' + fileInfo.bytes
      } else {
        const curFile = find(previewfix.value.fileList, {
          sysAttachFileId: file.sysAttachFileId
        })
        curFile.src = 'data:' + file.contentType + ';base64,' + fileInfo.bytes
      }

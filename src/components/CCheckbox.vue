    ...
    >
      <template v-slot:label v-if="label">
        <div class="row items-center all-pointer-events">
          <b>
            <span class="formLabelTitle" v-html="props.label" />
...
import { cloneDeep, split, clone, forEach, find, reject, filter } from 'lodash-es'

import { getComboItems } from '@utils/common'

import { GetTranLanguageFunction } from '@/types/language'
import { objectStringNumber, arrayObject, arrayString } from '@/types/variable'
...
function init() {
  if (
    ...
    if (!props.isFlag) {
      ...
      vValue.value = clone(props.value)
    }
  } else {
    if (!props.isFlag) {
      vValue.value = []
    } else {
      vValue.value = clone(props.falseValue)
    }
  }
...
function input() {
  ...
  if (!props.isFlag && vValue.value && Array.isArray(vValue.value) && vValue.value.length > 0) {
    forEach(vValue.value, (item) => {
      ...
      if (props.isObject) {
        // case 1
        const data: any = find(items.value, { value: item })
...
function makeSelectOptions(_items: arrayObject) {
  ...
  if (props.editable && !props.disabled) {
    ...
    tempcomboItems = reject(_items, { useFlag: 'N' })
  } else {
...
function setValue() {
  ...
  if (Array.isArray(props.value)) {
    tempValue = cloneDeep(props.value)
  } else {
    // String type으로 구분자가 ,로 들어오는 경우는 value값만
  ...
  if (items.value && items.value.length > 0) {
    ...
    if (!props.isFlag && tempValue && tempValue.length > 0) {
      forEach(tempValue, (item) => {
        ...
        if (props.isObject) {
          // case 1
          findData = find(items.value, { value: item[props.valueKey] })
        } else {
          // case 2
          findData = find(items.value, { value: item })
        }
        if (findData) {

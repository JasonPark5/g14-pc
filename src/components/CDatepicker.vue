 ...
 *  * 6. Etc (생길 시 얘기.)
 *******************************/
import { ref, computed, watch, inject, onBeforeMount, PropType } from 'vue'
...
import { clone, forEach, indexOf, replace } from 'lodash-es'
...
import { arrayString } from '@/types/variable'
...
function setValueText() {
  if (!props.range && !Array.isArray(value.value)) {
    valueText.value = clone(String(value.value))
  } else {
...
function setDefault() {
  if (!value.value || (value.value && value.value.length === 0)) {
    ...
    if (
      ...
      forEach(typeList, (item) => {
        if (indexOf(props.defaultStart, item) > -1) {
          start.type = item
          start.num = Number(replace(props.defaultStart, start.type, ''))
        }
        if (indexOf(props.defaultEnd, item) > -1) {
          end.type = item
          end.num = Number(replace(props.defaultEnd, end.type, ''))
        }
      })
  ...
  if (props.range && props.defaultStart === 'startOfYear' && props.defaultEnd === 'today') {
    value.value = [getFirstDayThisYear(), moment().format(format.value)]
  }
}
/******************************

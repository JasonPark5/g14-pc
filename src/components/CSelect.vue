...
import { find, reject, filter, forEach } from 'lodash-es'

import { getComboItems, getStepItems } from '@/common/utils/common'
 ...
 * @Emit_선언
 *******************************/
const emits = defineEmits(['update:value', 'datachange', 'setCodeData', 'setName'])
...
const valueText = computed(() => {
  ...
  if (items.value && items.value.length > 0 && vValue.value && !props.isSelect) {
    data = find(items.value, { value: vValue.value })
...
function input(paramValue: objectStringNumber) {
  ...
  emits('setName', items.value.find((item: any) => item.value === paramValue.value)?.label || '')
}
...
function makeSelectOptions(paramItems: arrayObject | undefined) {
  ...
  if (!paramItems || paramItems.length === 0) {
    ...
    if (props.rejectItems && props.rejectItems.length > 0) {
      forEach(props.rejectItems, (rejectItem: any) => {
        tempComboItems = reject(tempComboItems, (tempItem: any) => {

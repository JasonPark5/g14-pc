...
//   convertEnter: () => '',
//   /** Array, Object */
//   removeDuplicatedArray: () => [],
//   removeDuplicatedArrayObject: () => [],
//   /** file */
//   base64ToBlob: () => Blob,
//   bytesToSize: () => {},
//   getFileExtIcon: () => {},
...
export function getAddMonth(_date: string, _cnt: number) {
  if (!_date || !moment(_date, 'YYYY-MM-DD', true).isValid()) return ''
...
export function toBizNum(num: string | number): string {
  const cleanNum = String(num).replace(/[^0-9]/g, '') // 숫자만 추출
  if (cleanNum.length !== 10) return String(num) // 10자리 아니면 그대로 반환
  return cleanNum.replace(/(\d{3})(\d{2})(\d{5})/, '$1-$2-$3')
}
export function toMobileNum(num: string | number): string {
  const cleanNum = String(num).replace(/[^0-9]/g, '') // 숫자만 추출
  if (cleanNum.length !== 11) return String(num) // 10자리 아니면 그대로 반환
...
export function getComboItems(_codeGroupCd: string, _attrVal1?: string, useFlag?: string) {
  return new Promise((_resolve, _reject) => {
    ...
    $http({
      ...
      _resolve(filter(_result.data, { useFlag: useFlag || 'Y' }))
    })
  })
}
...
export async function getCodeName(_codeGroupCd: string, _cd: string): Promise<string> {
  const res: any = await $http({
    url: $format(selectConfig.sys.code.mst.get.url, _codeGroupCd, _cd),
    method: 'GET'
  })
  return res.data.codeName
}
...
export function limit(str: string, num: number) {

<template>
  <q-form ref="editForm">
    <c-card class="cardClassDetailForm passwordChangeArea">
      <template v-slot:card-title-custom>
        <!-- <div class="title text-center">
            <b>{{ $language('비밀번호 변경') }}</b>
          </div> -->
        <div class="subtitle" style="padding: 10px" v-html="subtitle" />
      </template>
      <template v-slot:card-detail>
        <div class="col-12">
          <c-text
            :required="true"
            label="기존 비밀번호"
            name="oripassword"
            type="password"
            autoComplete="off"
            v-model:value="userInfo.oripassword"
          />
        </div>
        <div class="col-12">
          <c-text
            :required="true"
            label="변경 비밀번호"
            name="password"
            type="password"
            autoComplete="off"
            v-model:value="userInfo.password"
          />
        </div>
        <div class="col-12">
          <c-text
            :required="true"
            label="변경 비밀번호 확인"
            name="passwordConf"
            type="password"
            autoComplete="off"
            v-model:value="userInfo.passwordConf"
          />
        </div>
        <div class="col-12" style="text-align: right">
          <c-btn label="비밀번호 변경" icon="save" @btnClicked="savePw" />
        </div>
      </template>
    </c-card>
  </q-form>
</template>

<script setup lang="ts">
/******************************
 * #Important 사용하지 않는 로직, 변수 등 선언 X
 *******************************/

/******************************
 * @import_선언
 * TODO 아래 순서에 맞추어 import (각 순서 마다 띄우기)
 *  * 1. Dependency
 *  * 2. Utils
 *  * 3. Types
 *  * 4. Stores
 *  * 5. Vue
 *  * 6. Etc (생길 시 얘기.)
 *******************************/
import { ref, computed, inject, onBeforeMount } from 'vue'
import { filter } from 'lodash-es'

import { encrypt } from '@/common/utils/aes256'
import { GetTranLanguageFunction } from '@/types/language'

import { storeToRefs } from 'pinia'
import { useLoadingStore } from '@stores/loading'

/******************************
 * @컴포넌트_옵션_선언
 * TODO 이름 정의 (파일 이름 그대로 지정)
 *******************************/
defineOptions({
  name: 'changePw'
})

/******************************
 * @Pinia_store_선언
 * TODO 반응형 유지를 위해 storeToRefs 사용 (function은 사용 X)
 *******************************/
const route = useRoute()
const login = useLoginStore()
const userStore = useUserStore()
const { user } = storeToRefs(userStore)

/******************************
 * @Emit_선언
 *******************************/

/******************************
 * @Vue_관련_선언 (ex. vue-router)
 *******************************/

/******************************
 * @Interface_선언
 *******************************/
interface propType {
  popupParam?: {
    message?: string
  }
}

/******************************
 * @inject_선언
 *******************************/
const $language = inject('$language') as GetTranLanguageFunction

/******************************
 * @Props_선언
 * TODO type & default 작성
 *******************************/
const props = withDefaults(defineProps<propType>(), {
  popupParam: () => {
    return {
      message: ''
    }
  }
})

/******************************
 * @VModel_선언
 *******************************/

/******************************
 * @Data_선언
 * TODO ref, reactive 사용, 불명확한 단어 사용 X (ex. data, date)
 *******************************/
/** message 호출 */
const message = useMessageStore()
const editable = ref(true)
const saveUrl = ref('')
const loading = useLoadingStore()

// eslint-disable-next-line vue/no-dupe-keys
const userInfo = ref({
  userId: '',
  oripassword: '',
  password: '',
  passwordConf: '',
  chgUserId: '',
  systemPw: ''
})

/******************************
 * @Computed_선언
 *******************************/
const subtitle = computed(() => {
  let text = `✔ ${$language('개인정보 보호를 위해 다른 사람이 쉽게 알아낼 수 있는 비밀번호는 사용하지 마세요.')}<br/>✔ ${$language('정기적인 비밀번호 변경으로 고객님의 개인정보를 보호해 주세요.')}<br/>❗❗ ${$language('비밀번호는 8자리 이상 소문자/대문자/숫자/특수문자 중 3개 이상 조합하여 작성하세요.')}`
  if (props.popupParam.message) text = `✔ ${props.popupParam.message} <br/>` + text
  return text
})

/******************************
 * @Watch_선언
 *******************************/

/******************************
 * @Life_cycle_선언
 *******************************/
onBeforeMount(() => {
  init()
})
/******************************
 * @Function_선언
 * TODO function 주석 작성 (asdffunctionannotation 사용)
 *  * arrow function 사용해도 무관
 *******************************/

/******************************
 * TODO (목적): 초기셋팅
 *******************************/
function init() {
  editable.value = Boolean(route.meta.editable)
  saveUrl.value = transactionConfig.mdm.user.changepw.url
  userInfo.value.userId = user.value.userId
  userInfo.value.chgUserId = user.value.userId
  userInfo.value.systemPw = user.value.password
}
/******************************
 * TODO (목적): 변경비밀번호 검증 후 저장
 *******************************/
function savePw() {
  const passwordReg = {
    lowerCase: /[a-z]/g,
    upperCase: /[A-Z]/g,
    numberCase: /[0-9]/g,
    /* eslint-disable-next-line */
    specialCase: /[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/g,
    lowerCaseTest: false,
    upperCaseTest: false,
    numberCaseTest: false,
    specialCaseTest: false
  }
  if (
    userInfo.value.oripassword === '' ||
    userInfo.value.password === '' ||
    userInfo.value.passwordConf === ''
  ) {
    message.alert({
      title: '안내',
      message: '비밀번호를 입력하세요!',
      type: 'warning' // success / info / warning / error
    })
    return
  } else if (userInfo.value.password !== userInfo.value.passwordConf) {
    message.alert({
      title: '안내',
      message: '변경 비밀번호와 확인 비밀번호가 다릅니다.',
      type: 'warning' // success / info / warning / error
    })
    return
  } else {
    // 비밀번호 조건 : 패스워드 길이 최소 8자리 이상 & 대문자/소문자/숫자/특주문자 중 3개 포함
    passwordReg.lowerCaseTest = passwordReg.lowerCase.test(userInfo.value.password)
    passwordReg.upperCaseTest = passwordReg.upperCase.test(userInfo.value.password)
    passwordReg.numberCaseTest = passwordReg.numberCase.test(userInfo.value.password)
    passwordReg.specialCaseTest = passwordReg.specialCase.test(userInfo.value.password)
    const resultTest = filter(passwordReg, (value) => value === false)
    if (resultTest.length > 1 || userInfo.value.password.length < 8) {
      message.alert({
        title: '안내',
        message:
          '비밀번호는 최소 8자리여야 하며,\n 대문자/소문자/숫자/특주문자 중 3개를 포함해야합니다.',
        type: 'warning' // success / info / warning / error
      })
      return
    } else {
      // 비밀번호 변경 확인 알림
      message.confirm({
        title: '확인',
        message: '변경하시겠습니까?\n (※ 변경되고 난 후 로그인 창으로 이동합니다.)',
        type: 'info', // success / info / warning / error
        // 확인 callback 함수
        confirmCallback: () => {
          $http({
            url: saveUrl.value,
            method: 'POST',
            data: {
              userId: userInfo.value.userId,
              oripassword: encrypt(userInfo.value.oripassword),
              password: encrypt(userInfo.value.password),
              passwordConf: encrypt(userInfo.value.passwordConf),
              chgUserId: userInfo.value.chgUserId
            }
          }).then((_result: any) => {
            if (_result.data === 'W') {
              message.alert({
                title: '안내',
                message: '기존 비밀번호가 틀립니다.',
                type: 'warning' // success / info / warning / error
              })
            } else {
              message.requestSuccess()
              login.LogOut().then(() => {
                loading.loadingShow()
                location.reload() // vue-router 객체 재설정을 위해
              })
              // emits('closePopup');
            }
          })
        },
        // 취소 callback 함수
        cancelCallback: () => {}
      })
    }
  }
}
</script>

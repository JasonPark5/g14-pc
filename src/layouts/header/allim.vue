        ...
        <q-card class="header-allim-layer">
          ...
          <q-card-section class="menu-search-content">
            ...
            <q-tab-panels v-model="allimtab" animated>
              ...
              <q-tab-panel v-if="tasks!.length > 0" name="work" class="q-my-sm">
                <q-list separator>
                  <q-item v-for="(task, idx) in tasks" v-ripple :key="idx" class="custom-hover">
                    <q-item-section avatar @click="goPage(task.pageLink)">
                      <q-avatar size="sm" color="primary" text-color="white">{{
                        task.cnt
                      }}</q-avatar>
                    </q-item-section>
                    <q-item-section>
                      <q-item-label
                        class="allim-txt q-mt-sm cursor-pointer"
                        @click.stop="
                          tasks.forEach((task, i) => {
                            task.expanded = i === idx ? !task.expanded : false
                          })
                        "
                      >
                        {{ task.title }}
                      </q-item-label>
                      <q-item-label v-show="task.expanded" caption class="text-left">
                        <span v-html="task.subTitle || ''" />
                      </q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
              ...
              <q-tab-panel v-else name="work" class="text-center q-my-xl">
                <b class="text-grey">알림이 없습니다.</b>
              </q-tab-panel>
...
function init() {
  ...
  // 프로젝트 시작시 주석 풀 것
  if (import.meta.env.MODE === 'production' || import.meta.env.MODE === 'otherproduction') {
    setInterval(() => {
      getApprCnt()
    }, 60000)
  }
...
function getTask() {
  $http({
    ...
    tasks.value = (_result.data || []).map((task: any) => ({
      ...task,
      expanded: false // ⭐ 토글용 상태 추가
    }))
  })
}

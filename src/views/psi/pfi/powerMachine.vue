...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.psi.pfi.power.list.url
  // code setting
  // list setting
  getList()
}
...
function linkClick(row?: any) {
  ...
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./powerMachineDetail.vue`))
  )
...
function excelUploadData() {
  popupOptions.value.title = '동력기계 업로드'
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./powerMachineExcelUpload.vue`))
  )
...
function closeExcelUploadPopup(_result: any) {
  ...
  if (_result && _result.length > 0) {
    ...
    $http({
      url: transactionConfig.psi.pfi.power.insert.url + '/excel',
      method: 'POST',
      data: s_data
    }).then(() => {

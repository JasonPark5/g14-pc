<template>
  <div>
    <c-card
      title="동력기계 목록 업로드 (동일한 동력기계번호의 데이터는 자료가 수정됩니다)"
...
const excelUploadInfo = ref<gridType>({
  rowKeys: ['psiPowerEquipmentNo'], // 데이터들의 키 속성값
  taskClassCd: 'PSI_POWER_MACHINE_UPLOAD', // 엑셀업로드 양식 구분 값
  sheetNum: 0,
  columns: [
    [
      {
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면, 동일한 동력기계번호의 데이터는 자료가 수정됩니다. 그대로 적용하시겠습니까?',

<template>
  <div>
    <c-card
      title="가스누출감지 경보기 설치계획 목록 업로드 (동일한 가스감지기NO는 자료가 수정됩니다.)"
 ...
 *******************************/
defineOptions({
  name: 'gasLeakDetectionAlarmInfoExcelUpload'
})
...
const excelUploadInfo = ref<gridType>({
  rowKeys: ['gasDetectorNo'], // 데이터들의 키 속성값
  taskClassCd: 'PIS_GAS_LEAK_DETECTION_UPLOAD', // 엑셀업로드 양식 구분 값
  sheetNum: 0,
  columns: [
    [
      {
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면, 동일한 가스감지기번호는 자료가 수정됩니다. 그대로 적용하시겠습니까?',

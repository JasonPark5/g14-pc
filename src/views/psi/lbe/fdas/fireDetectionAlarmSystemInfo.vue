  ...
  <div>
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <c-table
          ref="firedetection"
          title="화재탐지 및 경보설비 설치계획 목록"
          :columns="grid.columns"
          :data="grid.data"
          rowKey="fireDetectionId"
          :columnSetting="false"
          :expandAll="true"
          @linkClick="linkClick"
        >
 ...
 *******************************/
defineOptions({
  name: 'fireDetectionAlarmSystemInfo'
})
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.psi.lbe.firedetect.list.url
  // code setting
  // list setting
  getList()
}
...
function linkClick(row?: any) {
  popupOptions.value.title = '화재탐지 및 경보설비 설치계획 상세'
  popupOptions.value.param = {
    fireDetectionId: row ? row.fireDetectionId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./fireDetectionAlarmSystemInfoDetail.vue`))
  )
...
function excelUploadData() {
  popupOptions.value.title = '화재탐지 및 경보설비 설치계획 목록 업로드'
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./fireDetectionAlarmSystemInfoExcelUpload.vue`))
  )
...
function closeExcelUploadPopup(_result: any) {
  ...
  if (_result && _result.length > 0) {
    ...
    $http({
      url: transactionConfig.psi.lbe.firedetect.insert.url + '/excel',
      method: 'POST',
      data: s_data
    }).then(() => {

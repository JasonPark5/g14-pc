<template>
  <div>
    <c-card
      title="안전보호구/비상방재물품 목록 업로드 (동일한 관리번호는 자료가 수정됩니다.)"
 ...
 *******************************/
defineOptions({
  name: 'washingFacilityPlanSafetyExcelUpload'
})
  ...
  excelUploadInfo.value.columns = [
    [
      ...
      },
      {
        ...
        type: 'select',
        comboItems: [
          { code: 'SETC000001', codeName: $language('안전보호장구') },
          { code: 'SETC000002', codeName: $language('비상방재물품') }
        ],
        align: 'center',
        style: 'width:130px',
        sortable: false
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면, 동일한 관리번호의 데이터가 수정됩니다. 그대로 적용하시겠습니까?',

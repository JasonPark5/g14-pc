      ...
      <template v-slot:search>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
          <c-text
            ...
            v-model:value="searchParam.fireproofStructureName">
          </c-text>
        </div> -->
 ...
 *******************************/
defineOptions({
  name: 'fireResistantStructSpecInfo'
})
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.psi.lbe.fireproof.list.url
  // code setting
  // list setting
  getList()
}
...
function linkClick(row?: any) {
  popupOptions.value.title = '내화구조 명세 상세'
  popupOptions.value.param = {
    psiFireproofStructureId: row ? row.psiFireproofStructureId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./fireResistantStructSpecInfoDetail.vue`))
  )
...
function excelUploadData() {
  popupOptions.value.title = '내화구조 명세 업로드'
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./fireResistantStructSpecInfoExcelUpload.vue`))
  )
...
function closeExcelUploadPopup(_result: any) {
  ...
  if (_result && _result.length > 0) {
    ...
    $http({
      url: transactionConfig.psi.lbe.fireproof.insert.url + '/excel',
      method: 'POST',
      data: s_data
    }).then(() => {

<template>
  <div>
    <c-card
      title="내화구조명세 목록 업로드 (동일한 내화구조설비및지역명은 자료가 수정됩니다.)"
 ...
 *******************************/
defineOptions({
  name: 'fireResistantStructSpecInfoExcelUpload'
})
...
const excelUploadInfo = ref<gridType>({
  rowKeys: ['fireproofStructureName'], // 데이터들의 키 속성값
  taskClassCd: 'PSI_STRUCTURE_SPEC', // 엑셀업로드 양식 구분 값
  sheetNum: 0,
  columns: [
    [
      {
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면, 동일한 내화구조설비및지역명은 자료가 수정됩니다. 그대로 적용하시겠습니까?',

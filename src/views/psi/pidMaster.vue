<template>
  <div>
      ...
      <template v-slot:search>
        ...
        <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <c-text
            label="도면명"
            name="diagramTitle"
            v-model:value="searchParam.diagramTitle">
          </c-text>
        </div> -->
        <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <c-task-target
    ...
    <c-table
      ref="table"
      :title="pidInfo.title + ' ' + $language('목록')"
 ...
 *******************************/
function getList() {
  searchParam.value.diagramClassCd = props.pidInfo.diagramClassCd
  $http({
    url: listUrl.value,
    method: 'GET',
...
function linkClick(row?: any) {
  popupOptions.value.title = props.pidInfo.title + ' ' + $language('상세')
  popupOptions.value.param = {
    psiDiagramId: row ? row.psiDiagramId : null,
    diagramClassCd: props.pidInfo.diagramClassCd,
    title: props.pidInfo.title
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./pidMasterDetail.vue`))
  )

    ...
    <c-card :title="`[${map!.mapName}] 도면 정보`" class="cardClassDetailForm hazardequip-card">
      <template v-slot:card-description>
        <span style="color: tomato; margin-left: 10px">{{
          $language('도면이미지에 마우스 우클릭을 통해 토양오염대상시설 추가하세요.')
        }}</span>
      </template>
      ...
      <template v-slot:card-detail>
        ...
        >
          <div id="zoomable" ref="zoomable">
            ...
            <q-img
              id="zoomerImage"
              :src="map.mapSrc"
              :style="`width: ${(zoomer.height - 30) * (map.ratio || 1.4)}px; height: ${zoomer.height - 30}px;`"
            >
              <q-menu touch-position context-menu @before-show="beforeShow">
                <q-list dense style="min-width: 100px">
                  <q-item clickable v-close-popup @click="onItemClick">
                    ...
                    <q-item-section>{{ $language('토양오염대상시설명') }}</q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
              <c-map-marker
                v-for="(mark, idx) in map.facilities"
                :key="idx"
                :imgSrc="imgSrc"
              ...
              >
                <template v-slot:innerMark>
                  <q-menu
                    ...
                    self="top left"
                  >
                    <q-card flat bordered :class="['hazardequip-menu-card']">
        ...
        >
          <q-list bordered padding>
            <template v-for="(mapInfo, idx) in zoomer.facilities" :key="idx">
              ...
              <q-separator v-if="idx + 1 < zoomer.facilities.length" :key="idx + 'sep'" spaced />
            </template>
          </q-list>
        </div>
      </template>
...
import imgSrc from '@/assets/images/xxx.png'
...
const map = computed(() => {
  ...
  if (mapExists.value) {
    _map = _.find(zoomer.value.facilities, {
...
const mapExists = computed(
  () => zoomer.value.facilities!.length > 0 && Boolean(zoomer.value.currentMap!.sopMapId)
)
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.soil.map.list.url
  insertUrl.value = transactionConfig.env.soil.map.save.url
  deleteUrl.value = transactionConfig.env.soil.map.delete.url
  // code setting
  // list setting
  getList()
  setSize()
...
function getList() {
  $http({
    ...
    zoomer.value.facilities = _result.data || []

    if (zoomer.value.facilities!.length > 0 && !zoomer.value.currentMap?.sopMapId) {
      zoomer.value.currentMap!.sopMapId = zoomer.value.facilities![0].sopMapId
    }
  })
}
/******************************
...
function closeHazardStorage(data: any) {
  popupOptions.value.target = null
  popupOptions.value.visible = false
  if (data && data.length > 0) {
    let index = -1
    _.forEach(zoomer.value.facilities, (_item) => {
      if (index === -1) {
        index = _.findIndex(_item.facilities, {
          envSoilPollutionTargetFacilityId: data[0].envSoilPollutionTargetFacilityId
        })
      }
    })
    ...
    if (index === -1) {
      ...
      $http({
        ...
        data: {
          ...
          envSoilPollutionTargetFacilityId: item.envSoilPollutionTargetFacilityId, // 유해위험기계기구 일련번호
...
function removeClick(mark: any) {
  $http({
    url: $format(deleteUrl.value, mark.envSoilPollutionTargetFacilityMapId),
    method: 'DELETE'
  }).then(() => {
    message.requestSuccess()
    getList()

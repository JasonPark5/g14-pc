  ...
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
      <c-table
        ...
        :data="dailyReportData.preventiveModels"
        :usePaging="false"
        :columnSetting="false"
        :filtering="false"
 ...
 *******************************/
watch(
  () => props.dailyReportData?.preventiveModels,
  (rows) => {
    rowIndexMap.clear()
    if (!Array.isArray(rows)) return
    rows.forEach((r, i) => rowIndexMap.set(String(r.envWaterMstPreventiveId), i))
...
function tableDataChange(_props: any, col: any) {
  if (col.name === 'all') {
    for (let i = 0; i < 24; i++) {
      const idx = String(i).padStart(2, '0')
      _props.row[`oprTime${idx}`] = _props.row.all === 'Y' ? 'Y' : 'N'
    }
  } else if (col.name.startsWith('oprTime')) {
    updateAllFlag(_props.row)
  }

  const rows = props.dailyReportData?.preventiveModels
  if (!Array.isArray(rows) || rows.length === 0) return

  const rowKey = String(_props.row.envWaterMstPreventiveId)

...
const grid = ref<gridType>({
  columns: [
    ...
    },
    {
      ...
      field: 'iepsWaterSelfMeasureName',
      label: '자가측정명',
      align: 'center',
      type: 'link',
      sortable: true
    ...
    {
      name: 'envWaterMstOutletName',
      field: 'envWaterMstOutletName',
      label: '배출구명',
      align: 'center',
      style: 'width:20%',
      sortable: true
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.ieps.water.self.measure.list.url
  // code setting
  // list setting
  getList()
}
...
function linkClick(row?: any) {
  popupOptions.value.title = 'IEPS 수질 자가측정 상세'
  popupOptions.value.param = {
    envIepsWaterSelfMeasureMainId: row ? row.envIepsWaterSelfMeasureMainId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./selfWaterMeasureDetail.vue`))
  )

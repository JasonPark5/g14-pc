      ...
      child: [
        {
          ...
          sortable: false
          //type: 'select',
          //comboItems: materialItems.value
        },
        {
          //required: true,
          name: 'rawMaterialsUsage',
          field: 'rawMaterialsUsage',
          label: '원료사용량',
          align: 'right',
          style: 'width:120px',
          sortable: false
...
function init() {
  ...
  // url setting
  // 목록 조회
  listUrl.value = selectConfig.env.ieps.rawAndSub.list.url
...
function downLoadExcel() {
  $http({
    ...
    link.download = 'IEPS 원부원료사용량' + '(' + searchParam.value.usageMonth + ')' + '.xlsx'
    link.click()
  })
}
...
function linkClick(row: any) {
  popupOptions.value.title = '원부원료 사용량'
  popupOptions.value.param = {
    rawSubUsageId: row ? row.rawSubUsageId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./rawAndSubMaterialsUsageDetail.vue`))
  )

...
function getList() {
  ...
  if (searchParam.value.wasteYear !== null) {
    $http({
      ...
      if (!isAllSearch) {
        const sdata = _.filter(_result.data, { tvTypeCdTop: 'O', totalFlag: 'X' })
        chart.value.series = []
        ...
        chartHTML1.value.updateOptions({
          title: {
            text: $language('생산량-폐기물 원단위 추이분석')
          },
          yaxis: {
            title: { text: $language('원단위분석') }
          },
          xaxis: {
            categories: [
              $language('1월'),
        ...
        const sdata2 = _.filter(_result.data, { tvTypeCdTop: 'Q', totalFlag: 'X' })
        chart2.value.series = []
        ...
        chartHTML2.value.updateOptions({
          title: {
            text: $language('처리비용-폐기물 원단위 추이분석')
          },
          yaxis: {
            title: { text: $language('원단위분석') }
          },

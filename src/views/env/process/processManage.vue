          ...
          <c-card :title="title" class="cardClassDetailForm">
            <template v-slot:card-button>
              <q-btn-group outline>
                ...
                <c-btn
                  v-if="
                    editable && processData.processLevelCd === '10' && !selectedProcess.upProcessCd
                  "
                  label="신규 단위공정"
                  icon="add"
                  @btnClicked="addChild"
            ...
            <template v-if="processData.processLevelCd != '20'" v-slot:card-detail>
              ...
              <div class="col-4">
                <c-text
                  ...
                  label="순번"
                  name="sortOrder"
                  v-model:value="processData.sortOrder"
                />
              </div>
              <div class="col-4">
                <c-checkbox
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.process.list.url
  detailUrl.value = selectConfig.env.process.get.url
  checkUrl.value = selectConfig.env.process.check.url
  saveUrl.value = transactionConfig.env.process.save.url
  deleteUrl.value = transactionConfig.env.process.delete.url
  // code setting
  useFlagItems.value = [
      ...
      $http({
        ...
        if (updateMode.value || _result.data === 0) {
          message.confirm({
            ...
            // 확인 callback 함수
            confirmCallback: () => {
              processData.value.regUserId = user.value.userId

  ...
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
      <c-table
        ...
        :data="dailyReportData.preventiveOpertimes"
        :merge="grid.merge"
        :usePaging="false"
        :columnSetting="false"
        :filtering="false"
 ...
 *******************************/
watch(
  () => props.dailyReportData?.preventiveOpertimes,
  (rows) => {
    rowIndexMap.clear()
    if (!Array.isArray(rows)) return
    rows.forEach((r, i) => rowIndexMap.set(String(r.envAirMstPreventiveId), i))
...
function init() {
  // role setting
  // url setting
  // listUrl.value = selectConfig.env.air.daily.preventive.list.url
  // saveUrl.value = transactionConfig.env.air.daily.preventive.save.url
  // code setting
  // list setting
  // getList()
}
...
function tableDataChange(_props: any, col: any) {
  if (col.name === 'all') {
    for (let i = 0; i < 24; i++) {
      ...
      _props.row[`time${idx}`] = _props.row.all === 'Y' ? 'Y' : 'N'
    }
  } else if (col.name.startsWith('time')) {
    updateAllFlag(_props.row)
  }

  const rows = props.dailyReportData?.preventiveOpertimes
  if (!Array.isArray(rows) || rows.length === 0) return

  const rowKey = String(_props.row.envAirMstPreventiveId)

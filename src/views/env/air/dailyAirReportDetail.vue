          ...
          <c-card title="기본정보" class="cardClassDetailForm">
            <template v-slot:card-button>
              <q-btn-group outline>
                ...
                />
                <c-btn
                  v-if="dailyReport.approvalStatusCd === 'ASC9999999'"
                  label="일지출력"
                  color="amber-7"
                  icon="print"
                  @btnClicked="print"
                ...
                />
                <c-btn
                  v-if="editable && updateMode && !dailyReport.approvalStatusCd"
                  label="삭제"
                  icon="remove"
...
function init() {
  ...
  deleteUrl.value = transactionConfig.env.air.daily.report.delete.url
...
function deleteData() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.envAirDailyResultId),
        method: 'DELETE'
      }).then((_result: any) => {
        message.requestSuccess()
        emits('closePopup')

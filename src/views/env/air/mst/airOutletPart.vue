...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.air.mst.outlet.part.status
  // code setting
  // list setting
  getList()
}
...
function linkClick(row: any, col: any) {
  let tabname = ''
  if (col.name === 'envAirMstOutletName') tabname = 'govScheduleInfo'
  else if (col.name === 'envAirMstOutletPartName') tabname = 'govScheduleHistory'
  popupOptions.value.title = '대기 배출구(방지시설) 상세'
  popupOptions.value.param = {
    envAirMstPreventiveId: row.envAirMstOutletId,
    tabName: tabname
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./airMaster03Detail.vue`))
  )

    ...
    <c-search-box @enter="getList">
      <template v-slot:search>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
          <c-select
            :editable="editable"
            :comboItems="wattmeterFlagItems"
            label="전력계부착여부"
            itemText="codeName"
            itemValue="code"
            name="wattmeterFlag"
            type="search"
            v-model:value="searchParam.wattmeterFlag"
          />
        </div>
      </template>
    </c-search-box>
...
const wattmeterFlagItems = ref<Array<any>>([])
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.air.mst.preventive.list.url
  // code setting
  useFlagItems.value = [
    { code: 'Y', codeName: $language('사용') },
    { code: 'N', codeName: $language('미사용') }
  ]
  wattmeterFlagItems.value = [
    { code: 'Y', codeName: $language('부착') },
    { code: 'N', codeName: $language('미부착') }
  ]
  // list setting
  getList()
}
...
function linkClick(row: any) {
  ...
  popupOptions.value.param = {
    envAirMstPreventiveId: row ? row.envAirMstPreventiveId : ''
  }

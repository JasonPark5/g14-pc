      ...
      >
        <template v-slot:table-button>
          <q-btn-group outline>
            <c-btn
              label="추가"
              v-if="editable && selectedenvAirMstOutletPartId"
              icon="add"
              @btnClicked="addrow2"
            />
            <c-btn
              label="삭제"
              v-if="editable && selectedenvAirMstOutletPartId && grid2.data.length > 0"
              icon="remove"
...
const selectedenvAirMstOutletPartId = ref('')
const selectedenvAirMstOutletPartName = ref('')
const title = ref('')
const title2 = ref('부품별 교체/청소 이력')
const table1 = ref<any>(null)
const table2 = ref<any>(null)
const gridHeight = ref('780px')
...
function init() {
  ...
  listUrl.value = selectConfig.env.air.mst.outlet.part.url
  insertUrl.value = transactionConfig.env.air.mst.outlet.part.save.url
  deleteUrl.value = transactionConfig.env.air.mst.outlet.part.delete.url
...
function getDetail() {
  if (props.param.envAirMstPreventiveId) {
    $http({
      ...
      title.value =
        $language('방지시설 부품목록') + ' (' + _result.data.envAirMstPreventiveName + ')'
    })
  }
}
/******************************
...
function reset() {
  rowRemoveSelect()
  grid2.value.data = []
  selectedenvAirMstOutletPartId.value = ''
  selectedenvAirMstOutletPartName.value = ''
...
function getList() {
  $http({
    ...
    method: 'GET',
    params: {
      envAirMstOutletId: props.param.envAirMstPreventiveId
    }
  }).then((_result: any) => {
...
function rowClick(row: any) {
  selectedenvAirMstOutletPartId.value = row.envAirMstOutletPartId
  selectedenvAirMstOutletPartName.value = row.envAirMstOutletPartName
  title2.value =
    $language('부품별 교체/청소 이력') + ' (' + selectedenvAirMstOutletPartName.value + ')'
  $http({
    url: hislistUrl.value,
    method: 'GET',
    params: { envAirMstOutletPartId: row.envAirMstOutletPartId }
  }).then((_result: any) => {
...
function addrow1() {
  ...
  grid.value.data!.push({
    ...
    envAirMstOutletId: props.param.envAirMstPreventiveId, // 대기배출구(방지시설) 일련번호
...
function addrow2() {
  grid2.value.data!.push({
    ...
    plantCd: nowplantCd.value, // 사업장코드
    envAirMstOutletId: props.param.envAirMstPreventiveId, // 대기배출구(방지시설) 일련번호
    envAirMstOutletPartId: selectedenvAirMstOutletPartId.value, // 방지시설별 부품 일련번호

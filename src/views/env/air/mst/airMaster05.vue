...
const grid = ref<gridType>({
  columns: [
    ...
    {
      name: 'caloricValueUnitCd',
      field: 'caloricValueUnitCd',
      label: '발열량단위',
      align: 'center',
      style: 'width: 150px',
      sortable: true,
      type: 'select',
      codeGroupCd: 'CALORIC_VALUE_UNIT_CD'
    },
    {
...
const listUrl = ref('')
const saveUrl = ref(transactionConfig.env.air.mst.fuel.insert.url)
...
/******************************
 * TODO (목적): 테이블 값 변경에 따른 작업
 *******************************/
function tableDataChange(_props: any, _col: tableColumnType) {
  if (_col.name === 'airFuelUnitCd') {
    if (_props.row.airFuelUnitCd !== '14') {
      _props.row.airFuelUnitName = null
    }
  }
}

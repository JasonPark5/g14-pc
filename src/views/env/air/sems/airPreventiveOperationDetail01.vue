  ...
  <div>
    <q-form ref="editForm">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        ...
        <c-table
          ref="table"
          :title="`${searchParam.operationDate ? searchParam.operationDate : ''} 시설운전사항 목록`"
 ...
 *******************************/
defineOptions({
  name: 'airPreventiveOpertimeDetail01'
})
...
watch(
  ...
  () => {
    console.log(searchParam.value.envAirMstWattmeterId)
  }
)
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.env.air.sems.preventiveOperation.day.url
...
function getList() {
  if (
    !searchParam.value.operationDate ||
      ...
      message.confirm({
        ...
        confirmCallback: () => {
          _.forEach(grid.value.data, (item) => {
            if (!item.envAirSemsPreventiveOperationId) {
              item.editFlag = 'C'
            }

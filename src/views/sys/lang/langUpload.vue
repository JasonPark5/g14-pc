<template>
  <c-card title="다국어 데이터 일괄업로드" class="cardClassDetailForm">
    <template v-slot:card-button>
      <q-btn-group outline>
        <c-btn
          v-if="editable"
          label="다국어 데이터 양식 다운로드"
          icon="download"
...
interface langUploadType {
  sysLanguageId?: stringNull
  languageKey: stringNull
  languageKr: stringNull
  languageEn: stringNull
  languageJa: stringNull
  languageZhCn: stringNull
  languageZhTw: stringNull
...
const excelUploadInfo = ref<gridType>({
  rowKeys: ['sysLanguageId'], // 데이터들의 키 속성값
...
function init() {
  ...
  // url setting
  languageUrl.value = selectConfig.sys.language.down.url
  uploadUrl.value = transactionConfig.sys.language.save.url
  // code setting
  // list setting
}
/******************************
...
function downLoadTemplate() {
  $http({
    ...
    if (nav && nav.msSaveOrOpenBlob) {
      nav.msSaveOrOpenBlob(blob, '다국어업로드양식.xlsx')
    } else {
      const link = document.createElement('a')
      link.href = window.URL.createObjectURL(blob)
      link.download = '다국어업로드양식.xlsx'
      link.click()
    }
  })
}

<template>
  <div>
    <c-card
      title="성과평가 항목 업로드 (일괄업로드는 신규로만 저장이 됩니다.)"
...
const excelUploadInfo = ref<gridType>({
  rowKeys: ['evaluationItemId'], // 데이터들의 키 속성값
  taskClassCd: 'EVALUATION_ITEM_UPLOAD', // 엑셀업로드 양식 구분 값
  sheetNum: 0,
  columns: [],
  data: []
})
...
function init() {
  ...
  // url setting
  // code setting
  getComboItems('PERFORMANCE_EVALUATION_TYPE_CD').then((_result) => {
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면, 신규데이터로 저장됩니다. 그대로 적용하시겠습니까?',

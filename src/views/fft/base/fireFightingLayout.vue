      ...
      <div class="col-xs-12 col-sm-12 col-md-7 col-lg-7 col-xl-7">
        ...
        >
          ...
          <template v-slot:card-detail>
            <div class="col-12">
              <q-img :src="mapSrc" fit="contain" ref="image" :ratio="1.2">
 ...
 * @Computed_선언
 *******************************/
const setImgHeight = computed(() =>
  imgHeight.value.length > 0 ? Number(imgHeight.value.replace('px', '')) + 60 + 'px' : '800px'
)
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.fft.fireFightingMap.list.url
  // code setting
  useFlagItems.value = [
 ...
 *******************************/
function toggle() {
  $q.fullscreen.toggle(image.value.$el) // e.target.parentNode.parentNode.parentNode
}
/******************************
 * TODO (목적): 지도 상세 팝업 표시
 * @param (1): 행 정보
...
function linkClick(row?: any) {
  popupOptions.value.title = '배치도면 상세'
  popupOptions.value.param = { sopFireFightingMapId: row ? row.sopFireFightingMapId : '' }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./fireFightingLayoutDetail.vue`))
  )

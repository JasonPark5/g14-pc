      ...
      >
        ...
        <q-slide-transition v-if="!item.heading">
          <div v-show="item.visible">
            <component :is="item.component" :height="height" :param="param" />
          </div>
        </q-slide-transition>
      </q-timeline-entry>
    </q-timeline>
...
const tabHeight = computed(() => {
  const numHeight = props.height ? Number(_.replace(props.height, 'px', '')) + 130 : 600
  return numHeight + 'px'
})
  ...
  return [
    {
      title: '조사계획',
      visible: true,
      component: defineAsyncComponent(() => import(`./timeline/investigationPlanTL.vue`)),
      param: { disabled: true }
    },
    {
      title: '유해요인 조사',
      visible: true,
      component: defineAsyncComponent(() => import(`./timeline/hazardPlanTL.vue`)),
      param: { disabled: true }
    },
    {
      title: '증상설문조사 현황',
      visible: true,
      component: defineAsyncComponent(() => import(`./timeline/symptomSurveyStatusTL.vue`)),
      param: { disabled: true }
    }
  ] as any
})

...
function linkClick(row: any) {
  popupOptions.value.title = '직무스트레스 평가 상세'
  popupOptions.value.param = {
    heaJobStressPlanId: row ? row.heaJobStressPlanId : '',
    stepCd: row ? row.stepCd : 'HJS000001'
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./jobStressDetail.vue`))
  )

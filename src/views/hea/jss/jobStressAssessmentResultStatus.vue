  ...
  <div>
    <c-search-box>
      <template v-slot:search>
        ...
        <div v-if="searchParam.groupByDept" class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <c-select
            type="none"
            ...
            @update:value="(val: any) => dataChange(val, 1)"
          />
        </div>
        <div
          v-if="searchParam.groupByUpDept"
        ...
        >
          <c-select
            ...
            label="본부별"
            name="upDeptCd"
            v-model:value="searchParam.upDeptCd"
            @update:value="(val: any) => dataChange(val, 2)"
          />
        </div>
        <div
          v-if="searchParam.groupByJobclass"
          class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2"
        >
          <c-select
            type="none"
            :comboItems="plan.depts"
            itemText="jobClassNm"
            itemValue="jobClassCd"
            label="직종별"
            name="jobClassCd"
            v-model:value="searchParam.jobClassCd"
            @update:value="(val: any) => dataChange(val, 3)"
          />
        </div>
      </template>
    </c-search-box>
...
const chart = ref<apexChartType>({
  options: {
    ...
    legend: {
      ...
      customLegendItems: ['구분', '회사 중앙값(전체)', '한국 근로자 중앙값(전체)'],
      markers: {
...
function datachange() {
  ...
  emits('getDetail')
  console.log('depts', props.plan.depts)
}
/******************************
 * TODO (목적): 부서/본부/직종 선택값 변경
 *******************************/
function dataChange(_cd?: stringNull, num?: number) {
  if (_cd && props.plan.resultTable) {
    let _filter = []
    if (num === 1) _filter = _.filter(props.plan.resultTable, { deptCd: _cd })
    else if (num === 2) _filter = _.filter(props.plan.resultTable, { upDeptCd: _cd })
    else if (num === 3) _filter = _.filter(props.plan.resultTable, { jobClassCd: _cd })
    if (_filter.length > 0) {
      grid.value.data = _filter
      const items = _filter.map((item) => ({

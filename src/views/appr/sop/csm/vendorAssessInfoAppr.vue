    ...
    <q-form ref="editForm">
      <c-card title="협력업체 평가 기본정보" class="cardClassDetailForm">
        ...
        <template v-slot:card-description>
          <q-chip
            v-if="evaluation.evaluationGradeName"
 ...
 *  * 6. Etc (생길 시 얘기.)
 *******************************/
import { evalType } from '@views/sop/csm/portalIdChange'
...
const evaluation = ref<evalType>({
  ...
  vendorName: '',
  plantCd: null, // 사업장코드
  vendorEvaluationKindCd: null, // 협력업체 평가구분
...
function getDetail() {
  if (isOld.value) {
    ...
    $http({
      ...
      method: 'GET',
      params: { plantCd: evaluation.value.plantCd }
    }).then((_result: any) => {

 ...
 *******************************/
import { safetyGearType } from '@/views/sop/sas/safetyGear'
 ...
 *******************************/
function getDetail() {
  if (props.popupParam.requestGiveInfoId) {
    $http({
      url: $format(getUrl.value, props.popupParam.requestGiveInfoId),
      method: 'GET'
    }).then((_result: any) => {
...
function saveData() {
  if (safetyGear.value.requestGiveInfoId) {
    ...
    safetyGear.value.giveTypeCd = props.popupParam.giveTypeCd
...
function saveCallback(result: any) {
  props.popupParam.requestGiveInfoId = result.data
    ...
    _.forEach(_data, (item) => {
      if (_.findIndex(safetyGear.value.detailList, { safetyGearId: item.safetyGearId }) === -1) {
        safetyGear.value.detailList!.push({
          requestListId: uid(),
          requestGiveInfoId: props.popupParam.requestGiveInfoId,

 ...
 *******************************/
import { hpgCheckType } from '@views/sop/hpg/hpgCheck'
 ...
 *******************************/
watch(
  () => [hpgCheck.value.checkDate, hpgCheck.value.checkTypeCd, hpgCheck.value.checkerName],
  () => {
    hpgCheck.value.checkName = `고압가스 저장소 시설점검_${hpgCheck.value.checkTypeCd === 'D' ? '일간' : hpgCheck.value.checkTypeCd === 'W' ? '주간' : '월간'}_${hpgCheck.value.checkerName}_${hpgCheck.value.checkDate}`
  }
)
...
function getDetail() {
  if (props.popupParam.hpgCheckId) {
    ...
    hpgCheck.value.checkName = `고압가스 저장소 시설점검 일간_${user.value.userName}_${getToday()}`
    getItemList()
  }
}

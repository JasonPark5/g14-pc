 ...
 *  * 6. Etc (생길 시 얘기.)
 *******************************/
import { sapaInspectionType } from '@views/sai/sse/selfEvaluation'
 ...
 *******************************/
defineOptions({
  name: 'esteamInspectionDetailAppr'
})
  ...
  const items: Array<tabItemsType> = [
    {
      ...
      icon: 'edit',
      label: '기본정보',
      component: defineAsyncComponent(() => import(`@views/sai/sse/esteamInspectionDetail2.vue`)),
      key: uid()
    }
  ]
  if (isOld.value) {
...
const disabled = computed(
  () => inspection.value.completeFlag === 'Y' || inspection.value.approvalStatusCd === 'ASC9999999'
)

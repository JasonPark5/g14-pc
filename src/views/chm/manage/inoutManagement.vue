...
<template>
  <div>
    ...
    <c-table
      ref="table"
      title="유해화학자재 제조/수입/사용/판매 관리대장 (단위 : t)"
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.chm.inout.status.url
  // code setting
 ...
 * @param (1): 행 정보
 *******************************/
function linkClick(row?: inoutManagementType) {
  popupOptions.value.title = '유해화학자재 제조/수입/사용/판매 관리대장'
  popupOptions.value.param = {
    mdmChemMaterialId: row ? row.mdmChemMaterialId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./inoutManagementDetail.vue`))
  )

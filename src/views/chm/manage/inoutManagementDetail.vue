            ...
            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
              <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                  <div class="formLabelTitle txtlabel">{{ $language('제품 분류') }}</div>
                  <q-checkbox
                    :disable="true"
                    :editable="editable"
                    :label="$language('제조')"
                ...
                <div
                  ...
                  :key="idx"
                >
                  <div class="formLabelTitle txtlabel">{{ item.label }}</div>
                  <q-checkbox
    ...
    >
      ...
      <template v-slot:table-button>
        <q-btn-group outline>
          ...
          />
          <c-btn
            v-if="editable && !disabled && filterData.length > 0"
            label="삭제"
            icon="remove"
      ...
      <template v-slot:suffixTitle>
        <template v-if="filterData && filterData.length > 0">
          <!-- <span class="text-negative" style="font-size: 0.8em; font-weight: 300">
            ※ {{ $language('년월을 통해 월별 관리대장을 보실수 있습니다.') }}
          </span>
        </template>
        <template v-else> -->
          <c-select
            :comboItems="facilities"
            class="inout-facility no-label-control mx-10"
            type="search"
            itemText="facilityName"
            itemValue="facilityCd"
            name="facilityCd"
            label=""
            v-model:value="filter.facilityCd"
          />
          <span style="font-size: 0.8em; font-weight: 500; color: tomato">
            ※ {{ $language('Select box를 통해 시설을 필터링하여 보실수 있습니다.') }}
          </span>
        </template>
      </template>
...
import { inoutManagementType } from './inoutManagement'
...
interface gridInoutType {
  ...
  data: Array<inoutManagementType>
  prevData: Array<inoutManagementType>
}
...
const gridHeight = computed(() => {
  let height =
    props.contentHeight && !Number.isNaN(props.contentHeight)
      ? Number(props.contentHeight) - 335
      : 400
  if (height < 400) {
    height = 400
  }
...
const filterData = computed(() =>
  gridInout.value.data.filter(
    (data) =>
      !filter.value.facilityCd ||
      data.facilityCd.toLowerCase().includes(String(filter.value.facilityCd).toLowerCase())
  )
)
...
const facilities = computed(() => {
  ...
  if (gridInout.value.data && gridInout.value.data.length > 0) {
    const facilityCds = _.uniq(_.map(gridInout.value.data, 'facilityCd'))
    _.forEach(facilityCds, (facilityCd) => {
      if (facilityCd) {
        data.push({
          facilityCd: facilityCd,
          facilityName: _.find<inoutManagementType>(gridInout.value.data, {
            facilityCd: facilityCd
          })!.facilityName
        })
      }
...
function init() {
  ...
  listUrl.value = selectConfig.chm.inout.list.url
  yearMonthUrl.value = selectConfig.chm.inout.yearMonth.url
  saveUrl.value = transactionConfig.chm.inout.save.url
  deleteUrl.value = transactionConfig.chm.inout.delete.url
  printUrl.value = selectConfig.chm.inout.print.url
  // code setting
  setHeader()
  // list setting
  setInout()
...
function setHeader() {
  gridInout.value.columns = [
    ...
    {
      // 이후에 저장시설 관리 페이지 필요, 관리되는 데이터를 통해 관리대장 구성 및 허가량 관리
      name: 'facilityCd',
      field: 'facilityCd',
      label: '취급시설',
      align: 'left',
...
function getList(isPrev?: boolean) {
  ...
  $http({
    ...
    params: {
      ...
      facilities: !isPrev ? '' : _.map(facilities.value, 'facilityCd')
    }
  }).then((_result: any) => {
    if (!isPrev) {
      gridInout.value.data = _result.data.map((item: any) => ({
        ...item,
...
function tableDataChange(_props: any, col: tableColumnType) {
  if (col.name === 'inoutDate' || col.name === 'facilityCd') {
    // 1. 입출고일 2. 저장시설
    //   - 저장시설 있는지 확인 & 입출고일이 있는지 확인
    //   - 입출고일 & 저장시설 동일 데이터가 있는지 확인
    //   - 전 데이터 기준으로 재고량을 이월량 & 재고량으로 옮기기
    // ** 새로운 저장시설이 들어서는 경우 이전 데이터 중 최신 데이터를 찾는다.
    checkFacilityDate(_props, true)
    getList(true)
    ...
    checkFacilityDate(_props, false)
  }
}
/******************************
 * TODO (목적): 시설에 따른 날짜 유효성 검증
 * @param (1): 행 전체 정보
 * @param (2): 동일 날짜 여부 체크
 *******************************/
function checkFacilityDate(_props: any, sameDataCheck: boolean) {
  if (_props.row.facilityCd && _props.row.inoutDate) {
    if (sameDataCheck) {
      const filterLen = _.filter(gridInout.value.data, {
        facilityCd: _props.row.facilityCd,
        inoutDate: _props.row.inoutDate
      }).length
      if (filterLen > 1) {
        message.alert({
          title: '안내',
          message: '입출고일 & 저장시설이 동일한 데이터가 존재합니다.',
    ...
    if (filterData && filterData.length > 0) {
      ...
      if (_idx > 0) {
        ...
        const facilityIdx = _.findIndex(gridInout.value.prevData, {
          facilityCd: _props.row.facilityCd
        })
        if (gridInout.value.prevData && gridInout.value.prevData.length > 0 && facilityIdx > -1) {
          _props.row.carryover = Number(gridInout.value.prevData[facilityIdx].inventory)
          _props.row.inventory =
            Number(gridInout.value.prevData[facilityIdx].inventory) +
            Number(_props.row.inAmount) -
            Number(_props.row.outAmount)
        } else {
          // 첫 데이터
...
function add() {
  ...
  gridInout.value.data.splice(0, 0, {
    chmInoutManageId: uid(), // 입출고 번호
    mdmChemMaterialId: inout.value.mdmChemMaterialId, // 화학자재 일련번호
    materialCd: inout.value.mdmChemMaterialCd, // 자재코드
    yearmonth: '',
    inoutDate: '', // 입출고일
    facilityCd: filter.value.facilityCd ? filter.value.facilityCd : '', // 시설 코드
    facilityName: filter.value.facilityCd
      ? _.find(facilities.value, { facilityCd: filter.value.facilityCd })!.facilityName
      : '', // 시설명
    carryover: 0, // 이월량
    chmInTypeCd: null, // 입고구분코드
    inAmount: 0, // 입고량
    inAmountBefore: 0,
    inVendorCd: '', // 구입업체코드
    inVendorName: '', // 구입업체명
    ...
    inAddress: '', // 구입업체 주소
    inUserName: '', // 구입 담당자
    inPhoneNumber: '', // 구입 담당자 전화번호
    chmOutTypeCd: null, // 출고구분코드
    outAmount: 0, // 출고령
    outAmountBefore: 0, //
    outVendorCd: '', // 판매업체코드
    outVendorName: '', // 판매업체
    ...
    outAddress: '', // 판매업체 주소
    outUserName: '', // 판매 담당자
    outPhoneNumber: '', // 판매 담당자 전화번호
    outPurpose: '', // 용도
    outBuyerBusinessLicense: '', // 구매자 영업허가 구분
    inventory: 0, // 재고량
    remarks: '', // 비고
  ...
  if (!selectData || selectData.length === 0) {
    ...
    if (
      _.findIndex(filterData, (item) => {
        return !item.facilityCd || !item.inoutDate
      }) > -1
    ) {
      message.alert({
        title: '안내',
...
function allDataUpdate() {
   ...
   * 4. 저장
   */
  _.forEach(facilities.value, (facility) => {
    const splitData = _.sortBy(
      _.filter(gridInout.value.data, { facilityCd: facility.facilityCd }),
      'inoutDate'
    )
    if (splitData && splitData.length > 0) {
      ...
      _.forEach(splitData, (item) => {
        if (idx > 0) {
          ...
          if (facilityIdx > -1) {
            item.carryover = Number(gridInout.value.prevData[facilityIdx].inventory)
            item.inventory =
              Number(gridInout.value.prevData[facilityIdx].inventory) +
              Number(item.inAmount) -
              Number(item.outAmount)
          } else {
            item.carryover = 0
...
function print() {
  $http({
    ...
    link.download = inout.value.mdmChemMaterialName + '_' + inout.value.yearmonth + ' 관리대장.xlsx'
    link.click()
  })
}
/******************************

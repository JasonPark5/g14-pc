  ...
  const items: Array<tabItemsType> = [
    {
      ...
      icon: 'edit',
      label: '점검 정보',
      component: defineAsyncComponent(() => import(`./hazardFacilitySelfCheckInfo.vue`)),
      key: uid()
    }
  ]
  if (isOld.value) {
...
const disabled = computed(() => {
  return (
    // selfCheck.value.approvalStatusCd === 'ASC0000001' && !props.popupParam.isApprContent
...
function getDetail() {
  if (isOld.value) {
    $http({
      ...
      if (!_result.data.selfCheckUserId) _result.data.selfCheckUserId = user.value.userId
    ...
    $http({
      ...
      _.forEach(_result.data, (_item) => {
        selfCheck.value.selfCheckItemResults!.push({
          ...
          hcfAbnormalityCd: _item.hcfAbnormalityCd,
          remark: '',
          editFlag: 'C'
        })
      })

<template>
  <div>
    ...
    <c-table
      ref="table"
      title="유해화학자재 취급시설 자체점검 항목 목록 - 반기"
 ...
 *******************************/
defineOptions({
  name: 'hazardFacilitySelfSemiCheckItem'
})
...
function init() {
  ...
  // url setting
  // listUrl.value = selectConfig.env.facilitySelfSemiCheckItem.list.url
  // saveUrl.value = transactionConfig.env.facilitySelfSemiCheckItem.save.url
  listUrl.value = selectConfig.env.facilitySelfSemiCheckItem.list.url
  saveUrl.value = transactionConfig.env.facilitySelfSemiCheckItem.save.url
  // code setting
  useFlagItems.value = [
...
function add() {
  ...
  grid.value.data.splice(0, 0, {
    hcfHazardFacilitySelfCheckItemId: uid(), // 유해화학자재 취급시설 자체점검 항목 번호
    selfCheckItemType: '2', // 반기 2번
    selfCheckItemName: '', // 점검항목명
    useFlag: 'Y', // 사용여부

  ...
  <q-form ref="editForm">
    <c-card title="상세" class="cardClassDetailForm">
      ...
      <template v-slot:card-detail>
        ...
        <!-- 바꾸기 selfCheckItemType    v-model:value="searchParam.hcfSelfCheckCompleteFlag"-->
...
function init() {
  ...
  // 반기점검 기준 정보 항목 불러오기
  itemResultUrl.value = selectConfig.env.facilitySelfSemiCheckItem.list.url
  ...
  // code setting
  stepItems.value = [
    { code: '1', codeName: $language('주차 검사') },
    { code: '2', codeName: $language('반기 검사') }
  ]
  // list setting
  getDetail()
}

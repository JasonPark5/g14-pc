      ...
      <template v-slot:search>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
          <c-select
            ...
            label="제출 구분"
            v-model:value="searchParam.koraSubmitTypeCd"
          />
        </div>
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
          <c-select
            ...
            label="작성 수준"
            v-model:value="searchParam.koraSubmitLevelCd"
          />
        </div>
      </template>
    </c-search-box>
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.mdm.kora.document.list.url
  // code setting
  // list setting
  getList()
}
/******************************
 * TODO (목적): 화학사고 예방관리계획서 조회
 *******************************/
function getList() {
  if (period.value) {
...
/******************************
 * TODO (목적): 화학사고 예방관리계획서 링크 클릭
 * @param (1): 행 정보
 *******************************/
function linkClick(row: koraDocumentType) {
  bookPopup(row)
}
/******************************
 * TODO (목적): 화학사고 예방관리계획서 팝업 표시
 * @param (1): 행 정보
 *******************************/
function bookPopup(row: koraDocumentType) {
  popupOptions.value.title = '화학사고 예방관리계획서 상세'
  popupOptions.value.param = {
    koraDocumentId: row ? row.koraDocumentId : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./koraDocumentDetail.vue`))
  )
  popupOptions.value.visible = true
  popupOptions.value.closeCallback = closePopup
}
/******************************
 * TODO (목적): 화학사고 예방관리계획서 팝업 닫기
 *******************************/
function closePopup() {

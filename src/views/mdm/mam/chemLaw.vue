    ...
    <div class="col-xs-8 col-sm-8 col-md-8 col-lg-8 col-xl-8">
      <c-card title="법규별 규제사항" class="cardClassDetailForm">
        ...
        <template v-slot:card-button>
          <q-btn-group outline>
            <c-btn v-show="editable" label="이력보기" icon="restore" @btnClicked="openHistory" />
            <c-btn
              v-if="editable"
              label="규제항목 기준값 조회"
              icon="help"
            ...
            />
            <!-- <c-btn v-if="editable" label="검증" icon="check" @btnClicked="verification" /> -->
            <c-btn
              v-if="editable"
              :url="saveUrl"
              :isSubmit="isSave"
...
//     defineAsyncComponent(() => import(`./chemVerification.vue`))
//   )
//   popupOptions.value.visible = true
...
function saveInfo() {
  message.confirm({
    ...
    confirmCallback: () => {
      ...
      saveData.value.regUserId = user.value.userId
      saveData.value.reguls = _.filter(reguls.value, { val: 'Y' })
...
/******************************
 * TODO (목적): 화학자재 규제 이력보기
 *******************************/
function openHistory() {
  popupOptions.value.title = '화학자재 규제사항 이력 목록'
  popupOptions.value.param = {
    mdmChemMaterialId: props.popupParam.mdmChemMaterialId
  }
  popupOptions.value.target = shallowRef(defineAsyncComponent(() => import(`./chemLawHistory.vue`)))
  popupOptions.value.visible = true
  popupOptions.value.isFull = false

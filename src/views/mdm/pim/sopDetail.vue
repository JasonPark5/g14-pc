    ...
    return [
      {
        ...
        icon: 'info',
        label: '작업정보',
        component: defineAsyncComponent(() => import(`./sopInfo.vue`)),
        key: uid()
      },
      {
        name: 'additionalInfo',
        icon: 'playlist_add',
        label: '부가정보',
        component: defineAsyncComponent(() => import(`./sopAdditionalInfo.vue`)),
        key: uid(),
 ...
 *******************************/
watch(
  () => rev.value.isRevision,
  () => {
    tabOptions.value.additionalInfo.tabLabelStyle = rev.value.isRevision
...
function init() {
  ...
  // url setting
  detailUrl.value = selectConfig.mdm.sop.job.get.url
  revlistUrl.value = selectConfig.mdm.sop.job.revList.url
  // code setting
  // list setting
  getDetail()
}
function getDetail(_mdmSopId?: any) {
  if (isOld.value) {
    $http({
      url: $format(detailUrl.value, !_mdmSopId ? props.popupParam.mdmSopId : _mdmSopId),
      method: 'GET'
    }).then((_result: any) => {
      _.extend(sop.value, _result.data)
      sop.value.revSelectValue = _result.data.mdmSopId
    })
  }
}

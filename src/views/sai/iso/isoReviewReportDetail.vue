            ...
            <template v-slot:card-button>
              <q-btn-group outline>
                <c-appr-btn
                  ...
                  @beforeApprAction="approvalReport"
 ...
 * @Computed_선언
 *******************************/
const isOld = computed(() => Boolean(props.popupParam.reviewReportId))
const disabled = computed(
  () => isoReview.value.completeFlag === 'Y' || Boolean(isoReview.value.sysApprovalRequestId)
)
const approvalInfo = computed(() => {
  return {
    sysApprovalRequestId: isoReview.value.sysApprovalRequestId, // 결재요청번호
    /**
     * 결재상태코드
     * ASC0000001	결재중
     * ASC0000002	반려
     * ASC9999999	결재완료
     */
    approvalStatusCd: isoReview.value.approvalStatusCd,
    apprEditable: editable && isOld.value && isoReview.value.completeFlag !== 'Y', // 결재버튼 활성화 기준
    param: isoReview.value, // 결재 param
    approvalUrl: saveUrl.value, // 결재 url
    isApproval: isApproval.value, // 결재 submit
    approvalTypeCd: 'APTC000032', // 결재유형코드
    approvalParamValue: {
      // 결재상세화면 파라미터
      reviewReportId: isoReview.value.reviewReportId,
      isApprContent: true
    },
    approvalRequestName: '[경영검토 보고서] ' + isoReview.value.checkName, // 결재요청명 (문서 title)
    approvalConnId: isoReview.value.reviewReportId // 결재연결용 업무일련번호 (문서 key)
  }
})
...
function init() {
  ...
  deleteUrl.value = transactionConfig.sai.iso.report.delete.url
  // code setting
  // list setting
  getDetail()
}
...
function removeData() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.reviewReportId),
        method: 'DELETE'
      }).then((_result: any) => {

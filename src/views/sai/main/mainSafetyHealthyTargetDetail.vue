    ...
    <div class="row">
          ...
          <template v-slot:card-detail>
            ...
            <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
              <c-select
                ...
                label="구분"
                v-model:value="target.safetyHealthyTypeCd"
              />
            </div>
            <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
              <c-text
                ...
                label="제목"
                name="targetTitle"
                v-model:value="target.targetTitle"
              />
            </div>
            <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
              <c-datepicker
      ...
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
        <c-table
          ...
          :columnSetting="false"
          selection="multiple"
          rowKey="mainSafetyHealthyTargetPlanId"
        >
          <template v-slot:table-button>
            <q-btn-group outline>
              <c-btn v-if="editable" label="추가" icon="add" @btnClicked="addTargetPlan" />
              <c-btn
                v-if="editable && target.plans && target.plans.length > 0"
                label="제외"
                icon="remove"
 ...
 *******************************/
defineOptions({
  name: 'mainSafetyHealthyTargetDetail'
})
...
interface propType {
  popupParam: {
    mainSafetyHealthyTargetId: stringNull
  }
}
interface gridType {
...
const target = ref<any>({
  mainSafetyHealthyTargetId: '', // 안전보건목표 일련번호
  plantCd: null, // 사업장
  propelDeptName: '', // 추진부서
...
const isOld = computed(() => {
  return Boolean(props.popupParam.mainSafetyHealthyTargetId)
})
...
function init() {
  ...
  // url setting
  detailUrl.value = selectConfig.sai.mainsafetyHealthy.get.url
  saveUrl.value = transactionConfig.sai.mainsafetyHealthy.insert.url
  deleteUrl.value = transactionConfig.sai.mainsafetyHealthy.delete.url
  // code setting
  // list setting
  getDetail()
}
...
function getDetail() {
  if (isOld.value) {
    $http({
      url: $format(detailUrl.value, props.popupParam.mainSafetyHealthyTargetId),
      method: 'GET'
    }).then((_result: any) => {
...
function addTargetPlan() {
  popupOptions.value.title = '관계법령의무사항 목록'
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`@views/sai/main/mainSafetyHealthyMstPopup.vue`))
  )
...
/******************************
 * TODO (목적): 관계법령의무사항 팝업 닫기 callback
 *******************************/
function closeTargetPopup(data: any) {
  ...
  if (data && data.length > 0) {
    _.forEach(data, (item) => {
      if (
        _.findIndex(target.value.plans, { mainSafetyHealthyId: item.mainSafetyHealthyId }) === -1
      ) {
        target.value.plans.push({
          mainSafetyHealthyTargetPlanId: uid(), // 안전보건목표 계획 일련번호
          mainSafetyHealthyTargetId: props.popupParam.mainSafetyHealthyTargetId, // 안전보건목표 일련번호
          mainSafetyHealthyId: item.mainSafetyHealthyId, // 안전보건목표 마스터 일련번호
        ...
        _.forEach(selectData, (item) => {
          if (
            ...
            _.findIndex(target.value.deletePlans, {
              mainSafetyHealthyTargetPlanId: item.mainSafetyHealthyTargetPlanId
            })
          ) {
...
function openResult(row: any) {
  if (!row) return
  planRow.value = row
  popupOptions.value.title = row.targetName + ' ' + $language('상세')
  popupOptions.value.param = {
    mainSafetyHealthyTargetId: row.mainSafetyHealthyTargetId,
    mainSafetyHealthyTargetPlanId: row.mainSafetyHealthyTargetPlanId
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./mainSafetyHealthyTargetDetailPopup.vue`))
  )
  popupOptions.value.isFull = false
...
function saveTarget() {
  if (isOld.value) {
    ...
    saveUrl.value = transactionConfig.sai.mainsafetyHealthy.update.url
  } else {
...
function saveTargetCallback(result: any) {
  ...
  props.popupParam.mainSafetyHealthyTargetId = result.data
  getDetail()
}
/******************************
 * TODO (목적): 삭제
...
function deleteTarget() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.mainSafetyHealthyTargetId),
        method: 'DELETE'
      }).then((_result: any) => {
        message.requestSuccess()
        emits('closePopup')
...
/******************************
 * TODO (목적): 타 관계법령 의무사항 팝업
 *******************************/
function copyTarget() {
  popupOptions.value.title = '타 관계법령 의무사항 계획 검색'
  popupOptions.value.param = {
    type: 'single'
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./mainSafetyHealthyTargetPopup.vue`))
  )
  popupOptions.value.isFull = false
...
function closeCopyTargetPopup(data: any) {
  ...
  if (data && data.length > 0) {
    $http({
      url: $format(detailUrl.value, data[0].mainSafetyHealthyTargetId),
      method: 'GET'
    }).then((_result: any) => {
      ...
      _.forEach(_result.data.plans, (item) => {
        let _idx = _.findIndex(target.value.plans, {
          mainSafetyHealthyId: item.mainSafetyHealthyId
        })
        _idx = _idx > -1 ? _idx : 0
        target.value.plans.splice(_idx, 0, {
          mainSafetyHealthyTargetPlanId: uid(), // 안전보건목표 계획 일련번호
          mainSafetyHealthyTargetId: props.popupParam.mainSafetyHealthyTargetId, // 안전보건목표 일련번호
          mainSafetyHealthyId: item.saiSafetyHealthyId, // 안전보건목표 마스터 일련번호
          targetName: item.targetName, // 안전보건목표

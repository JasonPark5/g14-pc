 ...
 *******************************/
defineOptions({
  name: 'mainSafetyHealthyTargetDetailPopup'
})
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.sai.mainsafetyHealthy.result.list.url
  saveUrl.value = transactionConfig.sai.mainsafetyHealthy.result.save.url
  deleteUrl.value = transactionConfig.sai.mainsafetyHealthy.result.delete.url
  // code setting
  // list setting
  getColumn()
}
...
function getList() {
  $http({
    ...
    method: 'GET',
    params: {
      mainSafetyHealthyTargetPlanId: props.popupParam.mainSafetyHealthyTargetPlanId
    }
  }).then((_result: any) => {
...
function addResult() {
  grid.value.data.splice(0, 0, {
    ...
    mainSafetyHealthyTargetPlanId: props.popupParam.mainSafetyHealthyTargetPlanId, // 안전보건목표 계획 일련번호

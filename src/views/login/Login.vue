            ...
            <div class="wrap d-md-flex">
              <div class="text-wrap p-4 p-lg-5 text-center d-flex align-items-center order-md-last">
                <div class="text w-100 anivertilogo">
                  <h2>
                    <img
                      src="@images/group14_ci_w.png"
                      style="width: 120px; margin-right: 10px; position: relative; top: 6px"
                    />
                    EHS Portal
                  </h2>
                  <p style="font-size: 1.6em">(Environment, Health & Safety Management System)</p>
                </div>
              </div>
...
function onSubmit() {
  ...
  orderedPromise([
    ...
    { func: getUserDeptInfo } /** 언어별 라벨, 메시지 정보 get */
  ])
}
...
function actionLogin() {
  return new Promise((_resolve, _reject) => {
    ...
    login
      ...
      .catch((error) => {
        const errorCode = error?.response?.data?.returnCode || error.code
        const errorMessage = error?.response?.data?.message || error.message
        if (errorCode === 'INIT_PASSWORD') {
          changePassword(loginForm.userId)
          _resolve(false)
        } else if (errorCode === 'PASS_PASSWORD') {
          changePassword(errorMessage)
          _resolve(false)
        } else if (errorCode === 'UNDER_CONSTRUCTION') {

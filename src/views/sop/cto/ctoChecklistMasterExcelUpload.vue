<template>
  <div>
    <c-card
      title="행동관찰 체크리스트 업로드 (일괄업로드는 신규로만 저장이 됩니다.)"
 ...
 *******************************/
defineOptions({
  name: 'ctoChecklistMasterExcelUpload'
})
...
const excelUploadInfo = ref<gridType>({
  ...
  data: [],
  rowKeys: ['plantCd', 'mchecklistName', 'checklistName'],
  taskClassCd: 'CTO_CHECKLIST_UPLOAD', // 엑셀업로드 양식 구분 값
  sheetNum: 0
})
    ...
    if (!dataCntCheck) {
      ...
      if (errorDataCheck) {
        ...
        message.confirm({
          title: '확인',
          message:
            '해당 엑셀데이터를 업로드하면,\n\r신규데이터로 저장됩니다.(동일한 행동관찰 체크리스트는 사용안함 처리 후 업로드 하세요) \n\r그대로 적용하시겠습니까?',

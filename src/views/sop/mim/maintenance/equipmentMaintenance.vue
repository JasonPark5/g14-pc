      ...
      <template v-slot:search>
        ...
        <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <c-select
            ...
            label="정비유형"
            v-model:value="searchParam.equipmentCheckTypeCd"
          />
        </div>
        <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
          <c-select
            ...
            label="진행상태"
            v-model:value="searchParam.maintenanceStatusCd"
          />
        </div>
      </template>
    </c-search-box>
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.sop.min.maintenance.plan.list.url
  deleteUrl.value = transactionConfig.sop.min.maintenance.result.delete.url
  // code setting
  // if (props.popupParam.equipmentTypeCd) {
  //   searchParam.value.equipmentTypeCd = this.popupParam.equipmentTypeCd
  //   searchParam.value.equipmentCheckTypeCd = this.popupParam.equipmentCheckTypeCd
  //   searchParam.value.grade = this.popupParam.grade
  // }
  colorItems.value = [
...
function linkClick(row: any) {
  popupOptions.value.title =
    row.noPlanResultEnrollFlag === 'N' ? '설비정비결과 상세' : '무계획 설비정비결과 상세'
  popupOptions.value.param = {
    minEquipmentMaintenanceId: row.minEquipmentMaintenanceId ? row.minEquipmentMaintenanceId : '',
    selectData: []
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./equipmentMaintenanceResultDetail.vue`))
  )
...
function onItemClick(data: any) {
  ...
  if (data === 'A') {
    ...
    title = '설비정비계획 상세'
    popupOptions.value.target = shallowRef(
      defineAsyncComponent(() => import(`./equipmentMaintenancePlan.vue`))
    )
  ...
  if (data === 'C') {
    ...
    title = '무계획 정비 결과 상세'
    popupOptions.value.target = shallowRef(
      defineAsyncComponent(() => import(`./equipmentMaintenanceResult.vue`))
    )
    popupOptions.value.isFull = false
    popupOptions.value.width = '98%'
  }
...
function remove() {
  ...
  if (!selectData || selectData.length === 0) {
    ...
    message.confirm({
      ...
      type: 'warning', // success / info / warning / error
      confirmCallback: () => {
        selectData = _.reject(selectData, { maintenanceStatusCd: 'MCSC000015' })
        $http({
          url: deleteUrl.value,
          method: 'DELETE',

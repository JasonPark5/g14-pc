          ...
          <c-card title="일상점검 계획 기본정보" class="cardClassDetailForm">
            <template v-slot:card-button>
              <q-btn-group outline>
                ...
                />
                <c-btn
                  v-if="editable && !disabled && !dailyCheck.sysApprovalRequestId"
                  :url="saveUrl"
                  :isSubmit="isSave"
                  :param="dailyCheck"
            ...
            <template v-slot:card-detail>
              <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                <c-task-target
                  ...
                  v-model:value="dailyCheck.processCd"
                  @dataChange="
                    (newValue: string, oldValue: string, taskData: any) => datachange(taskData, 'A')
                  "
                />
              </div>
              <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                <c-datepicker
                  ...
                  type="month"
                  name="checkDate"
                  v-model:value="dailyCheck.checkDate"
                  @datachange="(taskData: any) => datachange(taskData, 'B')"
                />
              </div>
              <!-- <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                <c-plant
                  ...
                  :disabled="disabled || updateMode"
                  :editable="editable"
                  :plantCd="dailyCheck.plantCd"
                  type="edit"
                  name="plantCd"
                  v-model:value="dailyCheck.plantCd"
                />
              </div> -->
              <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                <c-dept
                  ...
                  label="점검부서"
                  name="checkDeptCd"
                  v-model:value="dailyCheck.checkDeptCd"
                />
              </div>
            </template>
          </c-card>
...
import { equipDailyPopupParamType, equipDailyType } from './equipmentDaily'
...
const dailyCheck = ref<equipDailyType>({
...
const tabItems = ref<Array<tabItemsType>>([
  {
    ...
    icon: 'list',
    label: '일상점검 결과',
    component: shallowRef(defineAsyncComponent(() => import(`./equipmentDailyResult.vue`)))
  },
  {
...
const imprTabInfo = computed(() => {
  return {
    ...
    requestContents: dailyCheck.value.dailyName,
    disabled:
      dailyCheck.value.checkStatusCd === 'MCSC000015' ||
      Boolean(dailyCheck.value.sysApprovalRequestId),
...
const tabHeight = computed(() => String(props.contentHeight - 150))
...
const isRemove = computed(() => {
  return props.popupParam.dailyCheckId && Boolean(!dailyCheck.value.sysApprovalRequestId)
})
const approvalInfo = computed(() => {
  return {
    sysApprovalRequestId: dailyCheck.value.sysApprovalRequestId, // 결재요청번호
    /**
     * 결재상태코드
     * ASC0000001	결재중
     * ASC0000002	반려
     * ASC9999999	결재완료
     */
    approvalStatusCd: dailyCheck.value.approvalStatusCd,
    apprEditable: dailyCheck.value.checkStatusCd && dailyCheck.value.checkStatusCd !== 'MCSC000015', // 결재버튼 활성화 기준
    param: dailyCheck.value, // 결재 param
    approvalUrl: saveUrl.value, // 결재 url
    isApproval: isApproval.value, // 결재 submit
    approvalTypeCd: 'APTC000003', // 결재유형코드
    approvalParamValue: {
      // 결재상세화면 파라미터
      dailyCheckId: dailyCheck.value.dailyCheckId,
      isApprContent: true
    },
    approvalRequestName: '일상점검 [' + dailyCheck.value.dailyName + ']', // 결재요청명 (문서 title)
    approvalConnId: props.popupParam.dailyCheckId // 결재연결용 업무일련번호 (문서 key)
  }
})
/******************************
 * @Watch_선언
...
function saveData() {
  mappingType.value = props.popupParam.dailyCheckId ? 'PUT' : 'POST'
  editForm.value.validate().then((_result: boolean) => {
    if (_result) {
      message.confirm({
        ...
        confirmCallback: () => {
          dailyCheck.value.regUserId = user.value.userId
          dailyCheck.value.chgUserId = user.value.userId

          dailyCheck.value.dailyResultModelList = _.clone(grid.value.data)

          if (mappingType.value === 'POST') {
            _.forEach(dailyCheck.value.dailyResultModelList, (_item) => {
              _item.editFlag = 'C'
            })
          }
          dailyCheck.value.utilityFlag = 'N'
...
function datachange(_data: any, type: string) {
  if (type === 'A' && dailyCheck.value.processCd) {
    dailyCheck.value.processName = _data.processName
    dailyCheck.value.processCd = _data.processCd
    $http({
      url: $format(resultItemDetail.value, dailyCheck.value.processCd),
      method: 'GET'
    }).then((_result: any) => {
      dailyCheck.value.dailyResultModelList = _result.data
      _.forEach(dailyCheck.value.dailyResultModelList, (_item: any) => {
        _item.dailyCheckResultId = uid()
      })
      setColumns()
    })
  } else if (type === 'B' && dailyCheck.value.checkDate) {
    // grid.value.data = [];
    setColumns()
  }
}
function setColumns() {
  ...
  const days: any = []
  const year: any = dailyCheck.value.checkDate.substring(0, 4)
  const month: any = dailyCheck.value.checkDate.substring(5)
  ...
  if (dailyCheck.value.checkDate) {
    $http({
      ...
      grid.value.data = _.clone(dailyCheck.value.dailyResultModelList)
    })
  }

  if (dailyCheck.value.checkDate && dailyCheck.value.processCd) {
    dailyCheck.value.dailyName = year + '년도  ' + month + '월  '
  }
}
...
function removeData() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.dailyCheckId),
        method: 'DELETE'
      }).then((_result: any) => {
        message.requestSuccess()
        emits('closePopup')

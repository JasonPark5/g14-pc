<template>
  <div>
    <q-form ref="editForm">
      <c-card title="관리감독자 점검일지 기본정보" class="cardClassDetailForm">
        ...
        <template v-slot:card-detail>
          ...
          <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
            <c-select
              ...
              label="점검구분"
              name="checkTypeCd"
              v-model:value="safetyCheck.checkTypeCd"
  ...
  const items: Array<tabItemsType> = [
    {
      ...
      icon: 'list',
      label: '안전점검일지 결과',
      component: defineAsyncComponent(() => import(`./safetyCheckResult.vue`)),
      key: uid()
    },
    {
...
const approvalInfo = computed(() => {
  return {
    sysApprovalRequestId: safetyCheck.value.sysApprovalRequestId, // 결재요청번호
    /**
     * 결재상태코드
     * ASC0000001	결재중
     * ASC0000002	반려
     * ASC9999999	결재완료
     */
    approvalStatusCd: safetyCheck.value.approvalStatusCd,
    apprEditable: editable.value && isOld.value && safetyCheck.value.checkStatusCd !== 'MCSC000015',
    param: safetyCheck.value, // 결재 param
    approvalUrl: saveUrl.value, // 결재 url
    isApproval: isApproval.value, // 결재 submit
    approvalTypeCd: 'APTC000008', // 결재유형코드
    approvalParamValue: {
      // 결재상세화면 파라미터
      safetyCheckId: safetyCheck.value.safetyCheckId,
      isApprContent: true
    },
    approvalRequestName: '안전점검일지 [' + safetyCheck.value.safetyCheckName + ']', // 결재요청명 (문서 title)
    approvalConnId: props.popupParam.safetyCheckId // 결재연결용 업무일련번호 (문서 key)
  }
})
...
const tabHeight = computed(() => {
  let height =
    props.contentHeight && !Number.isNaN(Number(props.contentHeight.replace('px', '')))
      ? Number(props.contentHeight.replace('px', '')) - 150
      : 200
 ...
 *******************************/
watch(
  () => [safetyCheck.value.checkDate, safetyCheck.value.checkDeptCd, safetyCheck.value.checkTypeCd],
  () => {
    if (!isOld.value) {
      const [_y, _m] = safetyCheck.value.checkDate.split('-')
      getCodeName('SAFETY_CHECK_TYPE_CD', safetyCheck.value.checkTypeCd).then((codeName) => {
        safetyCheck.value.safetyCheckName = `${_y}년 ${_m}월 ${safetyCheck.value.checkDeptName} ${codeName} 관리감독자 점검일지`
      })
    }
  }
)
...
function getDetail() {
  ...
  } else datachange(safetyCheck.value.checkTypeCd)
}
function datachange(val: any) {
  $http({
    url: resultItemUrl.value,
    method: 'GET',
    params: { useFlag: 'Y', checkTypeCd: val }
  }).then((_result: any) => {
...
function removeData() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.safetyCheckId),
        method: 'DELETE'
      }).then((_result: any) => {
        message.requestSuccess()
        emits('closePopup')

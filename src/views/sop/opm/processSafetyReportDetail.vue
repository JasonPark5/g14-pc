  ...
  <q-form ref="editForm">
    <div class="row">
      <div class="col-md-12 col-lg-12">
        ...
        <c-card title="공정안전보고서 기본정보" class="cardClassDetailForm">
          <template v-slot:card-select>
            <c-select
              ...
              label=""
              v-model:value="selectedOpmFolderDetailId"
          ...
          <template v-slot:card-button>
            <q-btn-group outline>
              <c-btn
                v-show="editable && popupParam.opmFolderDetailId && !isRevision"
                label="개정"
                icon="restart_alt"
              ...
              />
              <c-btn
                v-if="editable && popupParam.opmFolderDetailId"
                label="삭제"
                icon="remove"
        ...
        <div style="padding-top: 20px !important">
          <c-upload :attachInfo="attachInfo" :editable="editable" label="절차서 파일" />
        </div>
      </div>
    </div>
  </q-form>
 ...
 *******************************/
defineOptions({
  name: 'processSafetyReportDetail'
})
...
interface propType {
  popupParam: {
    opmFolderId?: stringNull
    opmFolderDetailId?: stringNull
  }
...
const selectedOpmFolderDetailId = ref<stringNull>('')
const getUrl = ref('')
const saveUrl = ref('')
const deleteUrl = ref('')
...
function init() {
  ...
  // url setting
  getUrl.value = selectConfig.sop.opm.folderDetail.get.url
  deleteUrl.value = transactionConfig.sop.opm.folderDetail.delete.url
  revlistUrl.value = selectConfig.sop.opm.folderDetail.rev.url
  saveUrl.value = transactionConfig.sop.opm.folderDetail.insert.url
  // code setting
  if (props.popupParam.opmFolderId) folderDetail.value.opmFolderId = props.popupParam.opmFolderId
  // list setting
  getData()
}
/******************************
...
function getData() {
  ...
  if (props.popupParam.opmFolderDetailId) {
    attachInfo.value.taskKey = props.popupParam.opmFolderDetailId
    getDetail(props.popupParam.opmFolderDetailId, true)
    selectedOpmFolderDetailId.value = props.popupParam.opmFolderDetailId
  }
}
function getDetail(_opmFolderDetailId?: stringNull, _check?: boolean) {
  $http({
    url: $format(getUrl.value, _opmFolderDetailId),
    method: 'GET'
  }).then((_result: any) => {
    folderDetail.value = _result.data
    folderDetail.value.regDtStr = folderDetail.value.regDtStr.slice(0, -2)
    isRevision.value = false
    if (_check) getRevList(_result.data.groupId)
    if (_result.data.opmFolderDetailId === props.popupParam.opmFolderDetailId) {
      editable.value = Boolean(route.meta.editable)
...
function saveCallback(result: any) {
  ...
  props.popupParam.opmFolderDetailId = result.data
...
function deleteData() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.opmFolderDetailId),
        method: 'DELETE'
      }).then(() => {
        message.requestSuccess()
        emits('closePopup')
...
function setRevision() {
  $http({
    ...
    attachInfo.value.beforeTaskKey = props.popupParam.opmFolderDetailId
...
function rowClickRev(row: any) {
  if (row && row !== folderDetail.value.opmFolderDetailId) {
    selectedOpmFolderDetailId.value = row
    getDetail(selectedOpmFolderDetailId.value, false)
    attachInfo.value.taskKey = selectedOpmFolderDetailId.value
  }
}

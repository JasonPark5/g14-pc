                    ...
                    >
                      <q-list dense class="topTabContext">
                        ...
                        <q-item clickable v-close-popup v-on:click="addFolder(prop.node)">
                          <q-item-section
                            ><q-icon class="text-dark" name="add" />{{
                              $language('하위 폴더 추가')
                            }}
                          </q-item-section>
                        </q-item>
                        <q-item clickable v-close-popup v-on:click="changeFolder(prop.node)">
                          <q-item-section
                            ><q-icon class="text-dark" name="published_with_changes" />{{
                              $language('폴더명 변경')
                            }}
                          </q-item-section>
                        </q-item>
                        <q-item clickable v-close-popup v-on:click="removeFolder(prop.node)">
                          <q-item-section
                            ><q-icon class="text-dark" name="delete" />{{ $language('폴더 삭제') }}
                          </q-item-section>
                        </q-item>
                      </q-list>
                    </q-menu>
          ...
          <template v-slot:table-chip>
            <q-btn-group outline>
              <q-chip outline square>
                <q-icon name="bookmark" class="text-primary" size="xs" style="margin-right: 5px" />
                {{ $language('폴더명') + ' : ' + folderName }}
              </q-chip>
            </q-btn-group>
          </template>
          <!-- 버튼 영역 -->
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.sop.opm.folder.list.url
  saveUrl.value = transactionConfig.sop.opm.folder.insert.url
  deleteUrl.value = transactionConfig.sop.opm.folder.delete.url
  listDetailUrl.value = selectConfig.sop.opm.folderDetail.list.url
  // code setting
  treeHeight.value = window.innerHeight - 270 + 'px'
  useFlagItems.value = [
...
function removeFolder(row: any) {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, row.opmFolderId),
        method: 'DELETE'
      }).then(() => {
        getList()
...
function saveFolderName() {
  ...
  $http({
    ...
    data: {
      opmFolderId: saveData.value.opmFolderId,
      opmParentFolderId: saveData.value.opmParentFolderId,
...
function rowClick(row: any) {
  opmFolderId.value = row.opmFolderId
  folderName.value = row.folderName
  $http({
    url: $format(listDetailUrl.value, row.opmFolderId),
    method: 'GET',
    params: searchParam.value
  }).then((_result: any) => {
...
function linkClick(row: any) {
  popupOptions.value.title = '공정안전보고서 상세'
  popupOptions.value.param = {
    opmFolderDetailId: row ? row.opmFolderDetailId : '',
    opmFolderId: opmFolderId.value ? opmFolderId.value : ''
  }
  popupOptions.value.target = shallowRef(
    defineAsyncComponent(() => import(`./processSafetyReportDetail.vue`))
  )
...
function closeDetailPopup() {
  ...
  rowClick({ opmFolderId: opmFolderId, folderName: folderName })
}

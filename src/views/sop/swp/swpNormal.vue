            ...
            >
              <template v-slot:table-button>
                <q-btn-group outline>
                  ...
                  />
                  <c-btn
                    v-if="editable && gasEditable && workPermit.gases.length > 0"
                    label="제외"
                    icon="remove"
                    :showLoading="false"
  ...
  $http({
    ...
    if (props.workPermit.gases && props.workPermit.gases.length > 0) {
      ...
      const useYList = _.filter(_result.data, (item) => {
        return (
          item.useFlag === 'Y' && _.findIndex(item.workTypes, { sopWorkTypeCd: 'SWT0000001' }) > -1
        )
      })
...
function addGas() {
  const pushData: any = {
    ...
    sopWorkPermitId: props.workPermit.sopWorkPermitId, // 작업허가서 일련번호
    supplementWorkTypeCd: 'SWT0000001', // 보충작업구분
    checkTime: '', // 점검시간
  ...
  props.workPermit.gases.splice(0, 0, pushData)
}
/******************************
 * TODO (목적): 가스 제거
 *******************************/

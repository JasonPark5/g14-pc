          ...
          <c-card title="기본정보" class="cardClassDetailForm work-card" :noMarginPadding="true">
              ...
              <q-btn-group outline>
                ...
                <c-btn
                  v-show="
                    ...
                    isWriting &&
                    isOld &&
                    !popupParam.isApprContent &&
                    !workPermit.sysApprovalRequestId &&
                    !vendorRequestFlag
                  "
                  label="삭제"
                  icon="remove"
            ...
            <template v-slot:card-detail>
              ...
              <template v-if="isWorkInfo">
                  ...
                  >
                    <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                      ...
                      >
                        ...
                        <q-separator />
                        <q-card-section class="no-padding row" style="padding: 5px !important">
                ...
                <div
                  class="col-12 folder-area text-center cursor-pointer bg-sub-color"
...
const tabHeight = computed(() => {
  let margin = 160
  if (isWorkInfo.value) margin += 30
  if (visible.value) margin = Number(margin) + 160
  else margin = Number(margin)
  const height =
    props.contentHeight && !Number.isNaN(props.contentHeight) ? Number(props.contentHeight) : 600
  return String(height - margin) + 'px'
})
...
const referencesCompo = computed(() => {
  return defineAsyncComponent(() => import(`./swpReferences.vue`))
})
...
const disabled = computed(
  () =>
    ...
    (workPermit.value.swpStepCd === 'SS00000015' ||
      ...
      // 결재중인 경우 비활성화
      workPermit.value.approvalStatusCd === 'ASC0000001')
)
const isWorking = computed(
  () =>
    ...
    // - 승인완료  - 연장
    isOld.value &&
    (workPermit.value.swpStepCd === 'SS00000005' ||
      workPermit.value.swpStepCd === 'SS00000010' ||
      workPermit.value.swpStepCd === 'SS00000020')
)
...
const vendorRequestFlag = computed(() =>
  workPermit.value.vendorFlag > 0 ? workPermit.value.swpWorkRequestCd : false
)
...
const approvalInfo = computed(() => {
  return {
    sysApprovalRequestId: workPermit.value.sysApprovalRequestId, // 결재요청번호
    /**
     * 결재상태코드
     * ASC0000001	결재중
     * ASC0000002	반려
     * ASC9999999	결재완료
     */
    approvalStatusCd: workPermit.value.approvalStatusCd,
    apprEditable:
      isOld.value &&
      workPermit.value.swpStepCd === 'SS00000001' &&
      (workPermit.value.vendorFlag > 0 ? workPermit.value.swpWorkRequestCd : true), // 결재버튼 활성화 기준
    param: workPermit.value, // 결재 param
    approvalUrl: approvalUrl.value, // 결재 url
    isApproval: isApproval.value, // 결재 submit
    approvalTypeCd: 'APTC000001', // 결재유형코드
    approvalParamValue: {
      // 결재상세화면 파라미터
      sopWorkPermitId: workPermit.value.sopWorkPermitId,
      permitTypeCd: workPermit.value.permitTypeCd,
      isApprContent: true
    },
    approvalRequestName: workPermit.value.workSummary, // 결재요청명 (문서 title)
    approvalConnId: workPermit.value.sopWorkPermitId // 결재연결용 업무일련번호 (문서 key)
  }
})
...
function init() {
  ...
  recentUrl.value = selectConfig.sop.swp.workPermit.recent.url
  ...
  approvalUrl.value = transactionConfig.sop.swp.workPermit.approval.url
  ...
  reqApprovalUrl.value = transactionConfig.sop.swp.workPermit.reqApproval.url
  printUrl.value = selectConfig.sop.swp.workPermit.print.url
  // code setting
  // list setting
  getDetail()
}
  ...
  if (props.popupParam.permitTypeCd === 'SPT0000001') {
    if (checkTab('normal')) {
      tabItems.value.push({
        ...
        class: 'tab-lightblue',
        label: '일반',
        component: shallowRef(defineAsyncComponent(() => import(`./swpNormal.vue`)))
      })
    }
  } else {
    if (checkTab('fire')) {
      tabItems.value.push({
        ...
        class: 'tab-red',
        label: '화기',
        component: shallowRef(defineAsyncComponent(() => import(`./swpFire.vue`)))
      })
    }
  }
        ...
        switch (supWork.supplementWorkTypeCd) {
          case 'SSWT000001':
            pushData = {
              ...
              icon: 'masks',
              label: '밀폐공간',
              component: shallowRef(defineAsyncComponent(() => import(`./swpConfinedSpace.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
          case 'SSWT000005':
            pushData = {
              ...
              icon: 'flash_off',
              label: '정전',
              component: shallowRef(defineAsyncComponent(() => import(`./swpBlackout.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
          case 'SSWT000010':
            pushData = {
              ...
              icon: 'warning',
              label: '방사선',
              component: shallowRef(defineAsyncComponent(() => import(`./swpRadiation.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
          case 'SSWT000015':
            pushData = {
              ...
              icon: 'hardware',
              label: '굴착',
              component: shallowRef(defineAsyncComponent(() => import(`./swpExcavation.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
          case 'SSWT000020':
            pushData = {
              ...
              icon: 'stairs',
              label: '고소',
              component: shallowRef(defineAsyncComponent(() => import(`./swpHighPlace.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
          case 'SSWT000025':
            pushData = {
              ...
              icon: 'agriculture',
              label: '중장비',
              component: shallowRef(defineAsyncComponent(() => import(`./swpHeavyEquipment.vue`))),
              supplementWorkTypeCd: supWork.supplementWorkTypeCd,
              supWork: supWork
            }
            break
 ...
 *******************************/
function closeWorkPermitPopup(data: Array<workPermitType>) {
  popupOptions.value.target = null
  popupOptions.value.visible = false
  if (data && data.length > 0) {
    $http({
      url: $format(detailUrl.value, data[0].sopWorkPermitId),
      method: 'GET',
      params: {
        load: 'true'
      }
 ...
 * TODO (목적): 업체요청 승인
 *******************************/
function reqApprovalWorkPermit() {
      ...
      message.confirm({
        ...
        confirmCallback: () => {
          const promises = [
            ...
            {
              func: type === 'save' ? saveValue : reqApprovalValue
            }
          ]
...
/******************************
 * TODO (목적): 반려 하기 전 데이터 셋팅
 *******************************/
function setSaveReturnValue() {
  return new Promise((resolve) => {
    workPermit.value.swpWorkRequestCd = ''
    resolve(true)
  })
}
/******************************
...
/******************************
 * TODO (목적): 작업허가서 승인 후 처리
 * @param (1): 작업허가서 id 담은 object
 *******************************/
function reqApprovalWorkPermitCallback(result: any) {
  message.requestSuccess()
  props.popupParam.sopWorkPermitId = result.data
  getDetail('save')
}
...
function removeWorkPermit() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.popupParam.sopWorkPermitId),
        method: 'DELETE'
      }).then(() => {
        message.requestSuccess()
        emits('closePopup')
...
/******************************
 * TODO (목적): 작업허가서 승인 결재 요청 전 처리
 *******************************/
function approvalWorkPermit() {
  /* 유효성 체크
   *  1. 발급정보
   */
  let isIng = true
  if (isIng) {
    if (!workPermit.value.issuedUserId || !workPermit.value.approvalUserId) {
      message.alert({
        title: '안내',
        message: '허가서의 발급정보가 입력되지 않았습니다.',
        type: 'warning' // success / info / warning / error
      })
      isIng = false
    }
 ...
 * TODO (목적): 결재 요청 팝업 표시
 *******************************/
function approvalWorkPermitCallback() {
...
function printWorkPermit() {
  $http({
    ...
    link.download = workPermit.value.workSummary + '.xlsx'
    link.click()
  })
}

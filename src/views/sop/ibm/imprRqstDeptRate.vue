                ...
                <li class="no-padding list-group-item">
                  <div class="widget-content p-0">
                    <div class="widget-content-outer">
                      <div class="widget-content-wrapper">
                        <div class="widget-content-left">
                          <div class="widget-heading">[{{ props.row.improveRequestDeptName }}]</div>
                          <div class="widget-subheading">
                            {{ $language('즉시처리건') }}
                          </div>
                        </div>
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.sop.ibm.improve.status.requestDept.url
  // code setting
  // list setting
  getList()
}
/******************************
 * TODO (목적): 개선 이행율(부서) 조회
 *******************************/
function getList() {
  $http({
    url: listUrl.value,
...
/******************************
 * TODO (목적): 개선 이행율(부서) 상세
 *******************************/
function linkClick(row: taskRateType) {
  popupOptions.value.title =
    $language('요청부서별 개선 이행률 상세') + ' [' + $language(row.ibmTaskTypeName) + ']'
  curSearchParam.value.ibmTaskTypeCd = row.ibmTaskTypeCd
  popupOptions.value.isFull = true
  popupOptions.value.param = {
    tab: 'improveRequestDeptName',
    clickInfo: row.improveRequestDeptName,
    data: row,

 ...
 *******************************/
function addTask() {
  popupOptions.value = getPopupOptions('workPermit', popupOptions.value, closeTaskPopup, {
    type: 'multiple'
  })
}
/******************************
 * TODO (목적): 작업허가서 추가 팝업 닫기 콜백
 *******************************/
function closeTaskPopup(data: any) {
  ...
  if (data && data.length > 0) {
    ...
    _.forEach(data, (item) => {
      if (_.findIndex(grid.value.data, { sopWorkPermitId: item.sopWorkPermitId }) === -1) {
        insertList.push({
          sopConstructionId: props.popupParam.sopConstructionId,
          sopWorkPermitId: item.sopWorkPermitId
        })

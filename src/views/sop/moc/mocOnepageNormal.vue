...
const items = computed(() => {
  const items = [
    {
      title: $language('판정'),
      visible: true,
      component: defineAsyncComponent(() => import(`./judgeCheckList.vue`))
    },
    {
      title: $language('발의'),
      visible: true,
  ...
  if (props.param.committeeHold && props.param.committeeHold.changeApprovalFlag === 'Y') {
    items.push({
      title: $language('실시'),
      visible: true,

<template>
  <div>
    <div v-show="deletable" class="col float-right">
      <q-btn-group outline>
...
const tabItems = ref([
  {
    ...
    icon: 'edit',
    label: '변경관리<br/>요구서',
    component: shallowRef(defineAsyncComponent(() => import(`./mocRequest.vue`)))
  },
  {
    name: 'category',
    icon: 'category',
    label: '검토항목',
    component: shallowRef(defineAsyncComponent(() => import(`./reviewItem.vue`)))
  }
])
...
const disabled2 = computed(() => {
  return props.param.mocTypeCd !== 'ME00000001' && Boolean(props.param.sopMocId)
})
...
function getDetail() {
  if (props.param.sopMocId) {
    ...
    moc.value.mocStepCd = 'ME00000001'
    props.param.mocStepCd = 'ME00000001'
  }
}
/******************************
 * TODO (목적): 저장 후 상태 값 변경 처리
 * @param (1): moc key
 *******************************/
...
function completeMoc() {
  ...
  let isProgress = true
  if (isProgress && !moc.value.mocConfirmLevelCd) {
    isProgress = false
    message.alert({
      title: '안내',
      // 등급(검토)를 입력하세요.
  ...
  if (isProgress) {
    message.confirm({
      ...
      confirmCallback: () => {
        ...
        moc.value.mocStepCd = 'ME00000005'
...
function removeMoc() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, moc.value.sopMocId),
        method: 'DELETE'
      }).then((_result: any) => {
        message.requestSuccess()
        emits('emitStep', {

...
const tabItems = ref([
  ...
  // { name: 'meeting', icon: 'question_answer', label: '위원회개최', component: () => import(`${'./mocMeeting.vue'}`) },
  {
    name: 'review',
    icon: 'preview',
    label: '검토/승인',
    component: shallowRef(defineAsyncComponent(() => import(`./mocReview.vue`)))
  }
  // { name: 'apply', icon: 'approval', label: '승인', component: () => import(`${'./mocApproval.vue'}`) },
])
 ...
 * @Computed_선언
 *******************************/
const approvalCheck = computed(() =>
  !moc.value.committeeHold || !moc.value.committeeHold.changeApprovalFlag ? false : true
)
...
const disabled2 = computed(() => {
  // 비상변경에 대해서는 생각하지 않음
  const stepCheck = props.param.mocTypeCd === 'MT00000001' ? 'MS00000005' : 'MT00000005'
  return props.param.sopMocId && approvalCheck.value && props.param.mocStepCd !== stepCheck
})

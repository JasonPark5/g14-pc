<template>
  <div>
          ...
          <template v-slot:card-button>
            <q-btn-group outline>
              <!-- <q-chip
                ...
                icon="report" 
                size="12px" >
                {{$language('개최정보가 하나 이상 등록 될 시')}} &nbsp;<strong>{{$language('유지/원복')}}</strong>&nbsp;{{$language('진행하실 수 있습니다.')}}
              </q-chip> -->
              <c-btn
                v-if="editable && !disabled2"
                :isSubmit="isSaveKeep"
                :url="keepComitteeHoldUrl"
                :param="moc"
                mappingType="PUT"
                label="유지"
                icon="approval"
                @beforeAction="keepComitteeHold"
                @btnCallback="keepComitteeHoldCallback"
              />
              <!-- 원복 -->
              <c-btn
                v-if="editable && !disabled2"
                :isSubmit="isSaveNonKeep"
                :url="keepComitteeHoldUrl"
                :param="moc"
                mappingType="PUT"
                label="원복"
                icon="do_not_disturb_on"
                @beforeAction="nonKeepComitteeHold"
                @btnCallback="nonKeepComitteeHoldCallback"
              />
              <c-btn
                v-if="editable && !disabled2"
    ...
    <c-table
      ...
      :gridHeight="gridRelatedWork.height"
...
const keepItems = ref<codeMasterType>([])
const isSaveComitteeHold = ref(false)
const isSaveKeep = ref(false)
const isSaveNonKeep = ref(false)
...
function init() {
  ...
  keepComitteeHoldUrl.value = transactionConfig.sop.moc.committeeHold.keep.url
  ...
  // code setting
  keepItems.value = [
    { code: 'Y', codeName: $language('유지') },
    { code: 'N', codeName: $language('원복') }
  ]
  // list setting
  setHoldData()
}
 ...
 * @return (반환):
 *******************************/
function checkKeep(flag: string) {
  // 위원회가 구성되었는지?
  // 개최정보가 최초/전문가/최종 중에 하나 이상 들어갔는지?
  // 원복 사유가 있는지?
  let isProgress = true
  // if (!props.moc.committees || props.moc.committees.length === 0) {
  //   isProgress = false;
  //   window.getApp.$emit('ALERT', {
  //     title: '안내',
  //     message: '위원회가 지정되지 않았습니다.', // 위원회가 지정되지 않았습니다.
  //     type: 'warning', // success / info / warning / error
  //   });
  //   return;
  // }
  // if (props.moc.committeeHold) {
  //   if (!props.moc.committeeHold.firstHoldDate // 최초
  //     && !props.moc.committeeHold.professionalHoldDate // 전문가
  //     && !props.moc.committeeHold.finalHoldDate) { // 최종
  //     isProgress = false;
  //     window.getApp.$emit('ALERT', {
  //       title: '안내',
  //       message: '위원회 회의가 개최되지 않았습니다.', // 위원회 회의가 개최되지 않았습니다.
  //       type: 'warning', // success / info / warning / error
  //     });
  //     return;
  //   }
  ...
  if (!props.moc.committeeHold!.changeApprovalDate) {
    isProgress = false
    message.alert({
      title: '안내',
      message: '유지/원복 결정일을 입력하시기 바랍니다.',
      type: 'warning' // success / info / warning / error
    })
    return
  }
  if (!props.moc.committeeHold!.changeKeepReason) {
    isProgress = false
    message.alert({
      title: '안내',
      message:
        (flag === 'keep' ? $language('유지') : $language('원복')) +
        $language('사유를 입력하시기 바랍니다.'),
      type: 'warning' // success / info / warning / error
    })
    return
  }
      ...
      if (_result) {
        message.confirm({
          ...
          confirmCallback: () => {
            ...
            props.moc.mocStepCd = 'ME00000010'
            props.moc.committeeHold!.chgUserId = user.value.userId
            props.moc.committeeHold!.changeKeepFlag = 'Y'

            isSaveKeep.value = !isSaveKeep.value
          },
          // 취소 callback 함수
          cancelCallback: () => {}
        })
 ...
 *******************************/
function keepComitteeHoldCallback() {
  message.requestSuccess()
 ...
 *******************************/
function nonKeepComitteeHold() {
  if (checkKeep('recovery')) {
    editForm.value.validate().then((_result: boolean) => {
      if (_result) {
        message.confirm({
          title: '확인',
          ...
          confirmCallback: () => {
            ...
            props.moc.mocStepCd = 'ME00000010'
            props.moc.committeeHold!.chgUserId = user.value.userId
            props.moc.committeeHold!.changeKeepFlag = 'N'

            isSaveNonKeep.value = !isSaveNonKeep.value
          },
          // 취소 callback 함수
          cancelCallback: () => {}
        })
 ...
 *******************************/
function nonKeepComitteeHoldCallback() {
  message.requestSuccess()
  emits('approvalCallback')
}
/******************************

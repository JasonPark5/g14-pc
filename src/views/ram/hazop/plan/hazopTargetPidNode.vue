          ...
          >
            <q-popup-proxy>
              <q-banner>
                ...
                />
                <span style="color: gray; margin-left: 10px">: {{ $language('노드 추가') }}</span>
              </q-banner>
            </q-popup-proxy>
          </q-chip>
          <q-btn-group outline>
...
function dataChange() {
  if (isRowAdd.value) {
    ...
    else isSameNodeName.value = false
  } else {
    if (
      _.findIndex(
...
function saveDrawNode() {
  ...
  _.forEach(props.processData.drawNodes, (_drawNode: any) => {
    if (_.findIndex(drawNodeSave.value.draws, { psiDiagramId: _drawNode.psiDiagramId }) === -1) {
      drawNodeSave.value.draws.push(_drawNode)
    }

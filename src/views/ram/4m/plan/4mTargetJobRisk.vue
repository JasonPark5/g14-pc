...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.ram.assessJob.list.url
  ...
  deleteUrl.value = transactionConfig.ram.assessJob.delete.url
  // code setting
  // list setting
  getDetail()
}
    ...
    if (_result.data && _result.data.length > 0) {
      ...
      _.forEach(tabItems.value, (item: any) => {
        ...
        item.component = shallowRef(
          defineAsyncComponent(() => import(`./4mTargetJobRiskDetail.vue`))
        )
        item.param = {
          fmScenarios: item.fmScenarios,

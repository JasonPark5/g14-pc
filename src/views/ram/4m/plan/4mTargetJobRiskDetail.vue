...
function init() {
  ...
  listUrl.value = selectConfig.ram.assessJob.fmRiskHazardList.url
  saveUrl.value = transactionConfig.ram.assessJob.fmRiskHazardSave.url
  // code setting
  getComboItems('RAM_RISK_HAZARD_CLASS_CD', 'fm').then((_result: any) => {
    typeList.value = _result
    getRiskHazardListUrl()
    if (props.popupParam.fmScenarios && props.popupParam.fmScenarios.length > 0) {
      inputDatas.value = _.uniq(_.map(props.popupParam.fmScenarios, 'ramRiskHazardId'))
    }
  })
  ...
  $http({
    ...
    _.forEach(typeList.value, (_item) => {
      grid.value.data.push({
        ram4mRiskHazardClassCd: _item.code,
        ram4mRiskHazardClassName: _item.codeName,
...
function getList() {
  $http({
    ...
    props.popupParam.fmScenarios = _result.data
    inputDatas.value = _.map(props.popupParam.fmScenarios, 'ramRiskHazardId')
  })
}
/******************************
 * TODO (목적):
...
function datachange(_props: any, _col: any, val: any[], item: any) {
  ...
  if (!isChecked) {
    const alreadyExist = saveData.value.deleteFmHazardList.some(
      (_result: any) => _result.ramRiskHazardId === item.ramRiskHazardId
    )
    if (!alreadyExist) {
      saveData.value.deleteFmHazardList.push({
    ...
    _.remove(
      saveData.value.deleteFmHazardList,
      (_result: any) => _result.ramRiskHazardId === item.ramRiskHazardId
    )
  }
}
 ...
 *******************************/
function saveSop() {
  if (validTable(grid.value.columns, props.popupParam.fmScenarios)) {
    message.confirm({
      title: '확인',
      message: '저장하시겠습니까?',
      ...
      // 확인 callback 함수
      confirmCallback: () => {
        saveData.value.fmHazardList = []
        _.forEach(inputDatas.value, (_item) => {
          const _hazardData = _.find(riskHazardList.value, { ramRiskHazardId: _item })
          if (_.findIndex(props.popupParam.fmScenarios, { ramRiskHazardId: _item }) === -1) {
            saveData.value.fmHazardList.push({
              ram4mAssessScenarioId: uid(),
              ramRiskAssessmentPlanId: props.assessPlan.ramRiskAssessmentPlanId,
              ...
              ram4mRiskHazardClassCd: _hazardData.ramRiskHazardClassCd, // 4M 분류 코드
              ram4mRiskHazardClassName: _hazardData.ramRiskHazardClassName, // 4M 분류

 ...
 * @Emit_선언
 *******************************/
const emits = defineEmits(['assessComplete', 'research', 'closePopup'])
...
const tabHeight = computed(() => {
  return String(window.innerHeight - 275) + 'px'
})
...
const isOld = computed(() => {
  return Boolean(props.param.ramRiskAssessmentPlanId)
})
...
function setSop() {
  if (props.processData && props.processData.sops && props.processData.sops.length > 0) {
    tab.value = props.processData.sops[0].ramJsaAssessJobId
  } else {
    tab.value = 'temp'
  }
}
function completeProcess() {
  ...
  let isProgress = true
  _.forEach(props.processData.sops, (sop) => {
    if (!sop.scenarios || sop.scenarios.length === 0) {
      isProgress = false
      message.alert({
        title: '안내',
        message: $language('작업에 등록된 시나리오가 없습니다.') + '[' + sop.jobName + ']',
  ...
  if (isProgress) {
    message.confirm({
      ...
      confirmCallback: () => {
        ...
        _.forEach(saveProcess.value.sops, (sop: any) => {
          delete sop.sop
        })
...
function removePlan() {
  message.confirm({
    ...
    confirmCallback: () => {
      $http({
        url: $format(deleteUrl.value, props.param.ramRiskAssessmentPlanId),
        method: 'DELETE'
      }).then(() => {
        message.requestSuccess()
        emits('closePopup')

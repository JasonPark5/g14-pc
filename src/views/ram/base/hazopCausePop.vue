<template>
  <c-table
    ref="table"
    :title="'이탈 (' + popupParam.deviationName + ') 별 가능한 원인 목록'"
...
function init() {
  ...
  // url setting
  listUrl.value = selectConfig.ram.variableCause.list.url
  // code setting
  // list setting
  getList()
}
...
function getList() {
  $http({
    ...
    method: 'GET',
    params: {
      ramVariableId: props.popupParam.ramVariableId,
      ramVariableGuidewordCd: props.popupParam.ramVariableGuidewordCd
    }
  }).then((_result: any) => {
...
function select() {
  ...
  if (!selectData || selectData.length === 0) {
    ...
    _.forEach(selectData, (item: any) => {
      item.deviationName = props.popupParam.deviationName
    })

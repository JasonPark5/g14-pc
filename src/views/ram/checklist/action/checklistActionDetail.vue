...
const tabHeight = computed(() => {
  return String(window.innerHeight - 265) + 'px'
})
...
function setTarget() {
  if (props.processData && props.processData.targets) {
    tab.value = props.processData.targets[0].ramAssessChecklistTargetId
  }
}
function completeProcess() {
  let isProgress = true
  _.forEach(props.processData.targets, (target) => {
    if (!target.scenarios || target.scenarios.length === 0) {
      isProgress = false
      message.alert({
        title: '안내',
        message:
          $language('평가대상에 등록된 시나리오가 없습니다.') +
          '[' +
          target.ramCheckItemClassName +
          ' / ' +
          target.checkItemName +
          ']',
  ...
  if (isProgress) {
    message.confirm({
      ...
      confirmCallback: () => {
        ...
        _.forEach(saveProcess.value.targets, (target: any) => {

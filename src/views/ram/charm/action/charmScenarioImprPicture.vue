...
interface propType {
  popupParam: {
    ramCharmResultAssessScenarioId: stringNull // 평가 일련번호
...
const props = withDefaults(defineProps<propType>(), {
  popupParam: () => {
    return {
      ramCharmResultAssessScenarioId: '', // 평가 일련번호
      sopImprovementIds: '',
      disabled: false
    }
  }
...
const attachBeforeInfo = ref<any>({
  isSubmit: '',
  task: []
})
...
function init() {
  ...
  if (props.popupParam.sopImprovementIds) {
    attachBeforeInfo.value.task = _.map(
      _.split(props.popupParam.sopImprovementIds, ','),
      (item) => {
        return {
